...
<body>
    ...
    <script>
        ...
        function editMaterial(materialId) {
            ...
            document.getElementById('availability').value = 'readily_available';
            
            document.getElementById('workability').value = 'good';
            document.getElementById('finishing').value = 'good';
            ...
            if (material.visual_assets?.texture_diffuse) {
            ...
            if (textureInfo) {
                textureInfo.innerHTML = "<strong>Texture Info:</strong><br>" +
                    "Main texture: " + (material.visual_assets?.texture_diffuse || "Not set") + "<br>" +
                    "Texture variants: " + (material.visual_assets?.texture_variants?.length || 0) + " variants";
            }
            }
        ...
        async function saveMaterial() {
            ...
            const material = {
                ...
                species: document.getElementById('scientific-name').value,
                visual_assets: {
                    // Preserve existing visual assets when editing
                    ...(editingMaterialId && materials[editingMaterialId]?.visual_assets || {})
                },
                physical_properties: {
                    density_lbs_ft3: parseFloat(document.getElementById('density').value) || 38,
                    hardness_janka: parseInt(document.getElementById('hardness').value) || 1000,
                    shrinkage_tangential: 7.8
                },
                default_configuration: {
                    grade: "select",
                    thickness: parseFloat(document.getElementById('thickness').value) || 0.75,
                    width: parseFloat(document.getElementById('width').value) || 6,
                    length: parseFloat(document.getElementById('length').value) || 96
                },
                cost_structure: {
                    base_price_bf: parseFloat(document.getElementById('price-per-bf').value) || 5.0,
                    freight_cost_bf: 0.75,
                    waste_factor: 0.15
                }
            };
        ...
        async function saveMaterialsToServer() {
            console.log("🚀 saveMaterialsToServer called");
