...
class ModernGizmoSystem {
    ...
    setupObservables() {
        ...
        if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERDOWN) {
            ...
            // Skip if gizmo already handled this
            if (pointerInfo.skipOnPointerObservable) return;
            
            // CRITICAL FIX: Skip if cut tool is active and should handle this
            ...
            const partPickInfo = this.scene.pick(x, y, (mesh) => {
                if (mesh) {
                }
            ...
            if (partPickInfo.hit && partPickInfo.pickedMesh && partPickInfo.pickedMesh.partData) {
                ...
                pointerInfo.skipOnPointerObservable = true; // Stop other handlers
            } else {
                // Clicked on empty space - check if not clicking a gizmo
                const gizmoPickInfo = this.scene.pick(x, y, (mesh) => mesh.isGizmo);
                if (!gizmoPickInfo.hit) {
                    // Really clicked empty space - deselect
                    if (this.selectedPart) {
                        this.deselectPart();
                    }
                }
    ...
    }, BABYLON.PointerEventTypes.POINTERDOWN, false, 500); // Priority 500 - after gizmo but before others // Priority 500 - after gizmo but before others
    ...
    handlePointerDown(pointerInfo) {
        ...
        // Do our own pick specifically for gizmos
        const pickInfo = this.scene.pick(
            this.scene.pointerX,
            this.scene.pointerY,
            (mesh) => mesh.isGizmo
        ...
        if (pickInfo.hit && pickInfo.pickedMesh.isGizmo && pickInfo.pickedMesh.gizmoHandle) {
    ...
    startDrag(handle, pickInfo) {
        ...
        // Calculate drag offset - CRITICAL FIX
...
class DrawingWorld {
    constructor() {
        ...
        // Part instances map - Single Source of Truth
        this.partInstances = new Map(); // Map of id -> Part instance
    ...
    initializeViewCube() {
        try {
            ...
            viewCubeContainer.id = 'viewCube';
            viewCubeContainer.style.position = 'absolute';
            viewCubeContainer.style.top = '10px';
            viewCubeContainer.style.right = '10px';
            viewCubeContainer.style.width = '100px';
            viewCubeContainer.style.height = '100px';
            viewCubeContainer.style.zIndex = '100';
            viewCubeContainer.style.backgroundColor = 'rgba(255, 255, 255, 0.9)';
            viewCubeContainer.style.border = '1px solid #ccc';
            viewCubeContainer.style.borderRadius = '5px';
            viewCubeContainer.style.cursor = 'pointer';
            
            if (canvasParent) {
                canvasParent.style.position = 'relative';
                canvasParent.appendChild(viewCubeContainer);
            }
            
            // Create canvas for ViewCube
            const viewCubeCanvas = document.createElement('canvas');
            viewCubeCanvas.width = 100;
            viewCubeCanvas.height = 100;
            viewCubeContainer.appendChild(viewCubeCanvas);
            
            // Create ViewCube scene
            const viewCubeEngine = new BABYLON.Engine(viewCubeCanvas, true);
            const viewCubeScene = new BABYLON.Scene(viewCubeEngine);
            viewCubeScene.clearColor = new BABYLON.Color4(0.95, 0.95, 0.95, 1);
            
            // Create ViewCube camera
            const viewCubeCamera = new BABYLON.ArcRotateCamera(
                "viewCubeCamera", 0, 0, 3,
                BABYLON.Vector3.Zero(),
                viewCubeScene
            );
            
            // Create cube with different colored faces for debugging
            const cube = BABYLON.MeshBuilder.CreateBox("viewCube", { size: 1 }, viewCubeScene);
            
            // Create multi-material with different colors per face
            const multiMat = new BABYLON.MultiMaterial("multi", viewCubeScene);
            ...
            cube.subMeshes = [];
            for (let i = 0; i < 6; i++) {
                cube.subMeshes.push(new BABYLON.SubMesh(i, 0, 4, i * 6, 6, cube));
            }
            
            cube.enableEdgesRendering();
            cube.edgesWidth = 4.0;
            cube.edgesColor = new BABYLON.Color4(0.4, 0.4, 0.4, 1);
            
            // Add light
            const light = new BABYLON.HemisphericLight("viewCubeLight",
                new BABYLON.Vector3(0, 1, 0), viewCubeScene);
            light.intensity = 0.8;
            
            // Sync cameras
            const syncCameras = () => {
                viewCubeCamera.alpha = this.camera.alpha;
                viewCubeCamera.beta = this.camera.beta;
            };
            ...
            viewCubeCanvas.addEventListener('pointermove', (e) => {
                ...
                const totalMove = Math.abs(e.clientX - startX) + Math.abs(e.clientY - startY);
            ...
            viewCubeCanvas.addEventListener('pointerup', (e) => {
                if (!isDragging) return;
                
                if (!hasDragged) {
                    // Click - detect face and animate to view
                    const rect = viewCubeCanvas.getBoundingClientRect();
                    const pickResult = viewCubeScene.pick(
                        e.clientX - rect.left,
                        e.clientY - rect.top
                    );
                    ...
                    if (pickResult.hit && pickResult.pickedMesh === cube) {
                        ...
                        const faceIndex = Math.floor(faceId / 2);
                        ...
                        switch(faceIndex) {
                            case 0: // RED side
                                targetAlpha = Math.PI / 2;  // Go to left view
                                targetBeta = Math.PI / 2;
                                break;
                            case 1: // GREEN side
                                targetAlpha = -Math.PI / 2;  // Go to right view
                                targetBeta = Math.PI / 2;
                                break;
                            case 2: // BLUE side
                                targetAlpha = 0;  // Front view
                                targetBeta = Math.PI / 2;
                                break;
                            case 3: // BLACK side (was incorrectly marked as bottom)
                        ...
                        while (deltaAlpha < -Math.PI) deltaAlpha += 2 * Math.PI;
                        const finalAlpha = this.camera.alpha + deltaAlpha;
            ...
            // Render loop
            viewCubeEngine.runRenderLoop(() => {
                syncCameras();
                viewCubeScene.render();
            });
            
            // Store references
            this.viewCubeEngine = viewCubeEngine;
            this.viewCubeScene = viewCubeScene;
            
            
        } catch (err) {
            console.error('Failed to initialize ViewCube:', err);
        }
    }
    ...
    selectSketchTool(toolName) {
        ...
        if (toolName === 'sketch') {
            ...
            // Update existing gizmo or create new one if part is selected
            if (this.gizmoManager && this.gizmoManager.attachedMesh) {
                // Gizmo already attached, just switch the type
                this.gizmoManager.positionGizmoEnabled = true;
                this.gizmoManager.rotationGizmoEnabled = false;
                this.gizmoManager.scaleGizmoEnabled = false;
            } else if (this.selectedPart) {
                // No gizmo yet, create one
            ...
            // Deactivate cut tool if it is active
            if (this.cutToolSystem) {
                this.cutToolSystem.deactivate();
            }
            ...
            // Update gizmo if part is selected
            if (this.selectedPart) {
                const selectedMesh = this.scene.meshes.find(m => m.partData === this.selectedPart);
                if (selectedMesh) {
                    this.createDragHandles(selectedMesh);
        ...
        try {
            ...
            if (!this.waitingForSurface && event.button === 0) {
                ...
                if (partPickInfo.hit && partPickInfo.pickedMesh && partPickInfo.pickedMesh.partData) {
                    ...
                    // Clicked on empty space - deselect part and hide gizmo
                    if (this.selectedPart) {
                        this.deselectPart();
                    }
                }
    ...
    setupEventListeners() {
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape') {
                ...
                // W key now selects move tool
                this.selectSketchTool('move');
            } else if (event.key === 'e' || event.key === 'E') {
                // E key now selects rotate tool
                this.selectSketchTool('rotate');
            } else if (event.key === 'r' || event.key === 'R') {
                // R key now selects scale tool if a part is selected
                if (this.selectedPart) {
                    this.selectSketchTool('scale');
                }
                // Router bit library is now in properties panel - show message
                else if (this.activeTool === 'router' && this.routerBitSystem) {
                    const selectionInfo = document.getElementById('selection-info');
                    if (selectionInfo) {
    ...
    closeMaterialModal() {
        this.isAddingMaterial = false; // Reset flag when closing modal
        const modal = document.getElementById('material-modal');
        if (modal) {
            modal.style.display = 'none';
        }
    ...
    createExtrusionGizmo() {
        ...
        this.extrusionGizmo.centerSphere = BABYLON.MeshBuilder.CreateSphere("extrusionCenter", {diameter: 0.2}, this.scene);
    ...
    setupMaterialModalListeners() {
        ...
        // Add to Project button (CRITICAL: This was missing!)
    ...
    addMaterialToProject() {
        if (!this.selectedMaterial || !this.materialsLibrary || !this.boardFactory) {
            ...
            this.isAddingMaterial = false; // CRITICAL: Reset flag on early return
            return;
        }
        ...
        try {
            ...
            // Create Part instance instead of plain object
            const materialData = this.materialsLibrary.getMaterial(this.selectedMaterial);
            
            const part = new Part({
                type: 'board',
                dimensions: board.getConventionalDimensions(), // Use Board's dimensions
                position: { x: 0, y: 0, z: 0 },
                rotation: { x: 0, y: 0, z: 0 },
                material: {
                    id: board.material.id,
                    name: board.material.species,
                    texture: materialData?.visual_assets?.texture_diffuse || null
                },
                ...
                _board: board  // Keep reference for now during transition
            });
            
            // Link Part to DrawingWorld for database saves
            part.drawingWorld = this;
            
            this.workBenchParts.push(part);
            // Link the existing board mesh to the Part instance
            if (board.mesh) {
                
                // Set critical properties for tool detection
                board.mesh.partInstance = part;
                board.mesh.partData = part;  // For backward compatibility
                board.mesh.isWorkBenchPart = true;
                board.mesh.isProjectPart = false;
                board.mesh.name = part.id;
                board.mesh.id = part.id;
                
                part.mesh = board.mesh;
                
                // Ensure the mesh is properly positioned above ground
                const halfThickness = (part.dimensions.thickness * 2.54) / 2;
                board.mesh.position.y = Math.max(halfThickness, board.mesh.position.y);
                
                // Update Part position to match mesh
                part.setPosition(board.mesh.position.x, board.mesh.position.y, board.mesh.position.z);
                
                // Make mesh pickable
                board.mesh.isPickable = true;
                
                // Force bounding box computation
                board.mesh.computeWorldMatrix(true);
                board.mesh.refreshBoundingInfo();
                
                // Add to workbench meshes array for tracking
                if (!this.workBenchMeshes) {
                    this.workBenchMeshes = [];
                }
                this.workBenchMeshes.push(board.mesh);
                
                
                // Setup gizmo observers for this mesh
                this.setupGizmoPartSaving(board.mesh);
            } else {
                console.error('No mesh found on board:', board);
            }
            // Close modal
            this.closeMaterialModal();
        ...
        if (this.camera.mode === BABYLON.Camera.ORTHOGRAPHIC_CAMERA && this.rulerContainer) {
            ...
            // DISABLED BoardFactory:             // The mesh is already created by BoardFactory
            // DISABLED BoardFactory:             // Position it appropriately
            // DISABLED BoardFactory:             const spawnPosition = this.findOptimalSpawnPosition(
            // DISABLED BoardFactory:                 board.dimensions.x * 2.54,
            // DISABLED BoardFactory:                 board.dimensions.y * 2.54,
            // DISABLED BoardFactory:                 board.dimensions.z * 2.54
            // DISABLED BoardFactory:             );
            // DISABLED BoardFactory:             board.transform.position = {
            // DISABLED BoardFactory:                 x: spawnPosition.x / 2.54,
            // DISABLED BoardFactory:                 y: spawnPosition.y / 2.54,
            // DISABLED BoardFactory:                 z: spawnPosition.z / 2.54
            // DISABLED BoardFactory:             };
            // DISABLED BoardFactory:             this.boardFactory.updateMeshTransform(board);
            // DISABLED BoardFactory:             
            // DISABLED BoardFactory:             // CRITICAL FIX: Set partData on the mesh so selection works!
            // DISABLED BoardFactory:             const mesh = this.scene.getMeshById(board.id);
            // DISABLED BoardFactory:             if (mesh) {
            // DISABLED BoardFactory:                 mesh.partData = part;  // Now the mesh can be selected and manipulated
            // DISABLED BoardFactory:                 part.mesh = mesh;  // Bidirectional link
            // DISABLED BoardFactory:                 console.log('✅ Set partData on mesh:', mesh.name, 'part:', part);
            // DISABLED BoardFactory:             }
            // DISABLED BoardFactory:             
            // DISABLED BoardFactory:             // Close modal
            // DISABLED BoardFactory:             
            // DISABLED BoardFactory:             
            // DISABLED BoardFactory:             console.log('✅ Material added to work bench using Board system:', board);
            
        } catch (error) {
            console.error("Failed to add material:", error);
            console.error("Error details:", error.message, error.stack);
            this.isAddingMaterial = false; // CRITICAL: Reset flag on error
            // alert removed - check console for errors
        } finally {
            // CRITICAL: Always reset flag regardless of success or failure
    ...
    createMaterialBlock(part) {
        ...
        const box = BABYLON.MeshBuilder.CreateBox(part.id, {
            width: lengthCm,     // Length → X-axis (match original board)
            height: thicknessCm, // Thickness → Y-axis
            depth: widthCm       // Width → Z-axis (match original board)
        }, this.scene);
                ...
                workBenchList.innerHTML = this.workBenchParts.map(part => {
                    ...
                    return `
                        <li class="part-item ${currentClass}" data-part-id="${part.id}" onclick="drawingWorld.focusPartInScene('${part.id}')">
                            ...
                            <div class="part-info">
                                <span class="part-name">${(part.materialName || (part.material ? part.material.name : "Unknown Material"))}</span>
    ...
    updateProjectPartsDisplay() {
        ...
        if (projectPartsList) {
            if (this.projectParts.length === 0) {
                // Method incomplete - adding closing braces
            }
        }
    }
    ...
    /**
     * Save workspace data to local storage or server
     */
    saveWorkspaceToLocal() {
        
        // Convert Part instances to plain objects for storage
        const workBenchData = [];
        for (const [id, partInstance] of this.partInstances) {
            workBenchData.push(partInstance.toPlainObject());
        }
        
        // Save to localStorage for now (can be changed to server save)
        const workspaceData = {
            workBenchParts: workBenchData,
            projectParts: this.projectParts,
            timestamp: Date.now()
        };
        
        try {
            localStorage.setItem('cutlist-workspace', JSON.stringify(workspaceData));
        } catch (error) {
    ...
    createWorkBenchMaterial(part, isRestoring = false) {
        ...
        // CRITICAL: Ensure material properties exist
        if (!part.materialId && part.material) {
            part.materialId = part.material.id || part.material.materialId;
        }
        if (!part.materialName && part.material) {
            part.materialName = part.material.name || part.material.materialName;
        }
        // Fallback to walnut if no material specified
        if (!part.materialId) {
            console.warn('No materialId found for part:', part.id, '- using default walnut');
            part.materialId = 'walnut';
            part.materialName = 'Black Walnut';
        }
        ...
        // Smart positioning: use saved position if available (for cut pieces or restored parts)
        ...
        // Always apply material with texture for consistency
        box.material = this.getMaterialColor(part.materialId || part.material?.id);

        // Store reference
        box.partData = part;
        
        // Create or retrieve Part instance for single source of truth
        let partInstance = this.partInstances.get(part.id);
        if (!partInstance) {
            if (part instanceof Part) {
                partInstance = part;
            } else {
                partInstance = Part.fromPlainObject(part, this);
            }
            this.partInstances.set(partInstance.id, partInstance);
        }
        
        // Link mesh to Part instance
        box.partInstance = partInstance;
        partInstance.mesh = box;
        
        // Sync mesh position/rotation to Part data (single source of truth)
        // partInstance.syncMeshToPartData(); - Method not needed, Part data already set
        ...
        // DEBUG: Check board properties for cutting system
...
//             this.animateCameraToShowcaseMaterial(box, part);
    ...
    sendPartBackToWorkBench() {
        ...
        this.workBenchParts.push(part);
            // CRITICAL: Create mesh for Part using unified system
            this.createWorkBenchMaterial(part);
            // Close modal
    ...
    selectPart(part) {
        ...
        if (mesh) {
            ...
            // Create gizmos for manipulation tools (move, rotate, scale)
            if (this.activeTool === 'move' || this.activeTool === 'rotate' || this.activeTool === 'scale') {
    ...
    setupGizmoPartSaving(mesh) {
        if (!mesh || !mesh.partInstance) {
            console.warn('setupGizmoPartSaving: No Part instance on mesh');
            return;
        }

        // Check if gizmo manager is ready
        if (!this.gizmoManager || !this.gizmoManager.gizmos) {
            return;
        }
        ...
        if (this.partGizmoObservers) {
            this.partGizmoObservers.forEach(observer => {
                if (observer.observable && observer.observer) {
                    observer.observable.remove(observer.observer);
                }
            });
        }
        this.partGizmoObservers = [];

        // Position gizmo observer
        if (this.gizmoManager.gizmos.positionGizmo) {
            const positionObserver = this.gizmoManager.gizmos.positionGizmo.onDragEndObservable.add(() => {
                part.setPosition(mesh.position.x, mesh.position.y, mesh.position.z);
            });
            this.partGizmoObservers.push({
                observable: this.gizmoManager.gizmos.positionGizmo.onDragEndObservable,
        ...
        if (this.gizmoManager.gizmos.rotationGizmo) {
            const rotationObserver = this.gizmoManager.gizmos.rotationGizmo.onDragEndObservable.add(() => {
                part.setRotation(mesh.rotation.x, mesh.rotation.y, mesh.rotation.z);
            });
            this.partGizmoObservers.push({
                observable: this.gizmoManager.gizmos.rotationGizmo.onDragEndObservable,
        ...
        if (this.gizmoManager.gizmos.scaleGizmo) {
            const scaleObserver = this.gizmoManager.gizmos.scaleGizmo.onDragEndObservable.add(() => {
                // Calculate new dimensions based on scale
                const newLength = part.dimensions.length * mesh.scaling.z;
                const newWidth = part.dimensions.width * mesh.scaling.x; 
                const newThickness = part.dimensions.thickness * mesh.scaling.y;
                part.setDimensions(newLength, newWidth, newThickness);
                
                // Reset mesh scaling since dimensions are now updated
                mesh.scaling.setAll(1);
            });
            this.partGizmoObservers.push({
                observable: this.gizmoManager.gizmos.scaleGizmo.onDragEndObservable,
        ...
        // Use GizmoManager - it handles everything!
        // Create utility layer first so GizmoManager can use it
        if (!this.gizmoUtilityLayer) {
            ...
            this.gizmoUtilityLayer.shouldRender = true;
        }
        
        if (!this.gizmoManager) {
            this.gizmoManager = new BABYLON.GizmoManager(this.scene, 1, this.gizmoUtilityLayer);
        ...
        // Enable the appropriate gizmo BEFORE attaching to mesh
        if (this.activeTool === 'move') {
            this.gizmoManager.positionGizmoEnabled = true;
            this.gizmoManager.rotationGizmoEnabled = false;
            this.gizmoManager.scaleGizmoEnabled = false;
        ...
        // Setup Part instance saving for gizmos if this mesh has a Part
        if (mesh.partInstance) {
            this.setupGizmoPartSaving(mesh);
        }
        ...
        // DISABLED OLD SYSTEM: this.setupGizmoTransformSaving(mesh);
...
// Simple approach: Just watch for mesh changes
        ...
        // Gizmo already enabled before attach
...
// DIRECT GIZMO TEST
window.testGizmoDirectly = () => {
    
    // Get the first workbench part mesh
    const mesh = drawingWorld.scene.meshes.find(m => m.isWorkBenchPart);
    if (!mesh) {
        return;
    }
    
    
    // Create a brand new GizmoManager
    if (drawingWorld.testGM) {
        drawingWorld.testGM.dispose();
    }
    
    drawingWorld.testGM = new BABYLON.GizmoManager(drawingWorld.scene);
    drawingWorld.testGM.usePointerToAttachGizmos = false;
    
    // Enable position gizmo FIRST
    drawingWorld.testGM.positionGizmoEnabled = true;
    
    // THEN attach to mesh
    drawingWorld.testGM.attachToMesh(mesh);
    
    
    return drawingWorld.testGM;
};

// TEST WITH MANUAL GIZMO CREATION
window.testManualGizmo = () => {
    ...
    // Dispose old gizmo if exists
    if (drawingWorld.manualGizmo) {
        drawingWorld.manualGizmo.dispose();
    }
    
    // Create position gizmo directly
    drawingWorld.manualGizmo = new BABYLON.PositionGizmo();
    drawingWorld.manualGizmo.attachedMesh = mesh;
    drawingWorld.manualGizmo.scaleRatio = 2;
    
    // Update gizmo to show  
    drawingWorld.manualGizmo.updateGizmoRotationToMatchAttachedMesh = true;
    drawingWorld.manualGizmo.updateGizmoPositionToMatchAttachedMesh = true;
    
    
    return drawingWorld.manualGizmo;
};

// TEST GIZMO WITH UTILITY LAYER
window.testGizmoWithLayer = () => {
    ...
    // Create utility layer if it doesn't exist
    if (!drawingWorld.utilityLayer) {
        drawingWorld.utilityLayer = new BABYLON.UtilityLayerRenderer(drawingWorld.scene);
    }
    
    // Dispose old gizmo if exists
    if (drawingWorld.testGizmoWL) {
        drawingWorld.testGizmoWL.dispose();
    }
    
    // Create position gizmo with utility layer
    drawingWorld.testGizmoWL = new BABYLON.PositionGizmo(drawingWorld.utilityLayer);
    drawingWorld.testGizmoWL.attachedMesh = mesh;
    drawingWorld.testGizmoWL.scaleRatio = 2;
    
    // Force update
    drawingWorld.testGizmoWL.updateGizmoRotationToMatchAttachedMesh = true;
    drawingWorld.testGizmoWL.updateGizmoPositionToMatchAttachedMesh = true;
    
    
    // Also check visibility
    if (drawingWorld.testGizmoWL._rootMesh) {
    }
    
    return drawingWorld.testGizmoWL;
};

// WORKING GIZMO IMPLEMENTATION
window.createWorkingGizmo = () => {
    ...
    // Dispose any existing test gizmos
    if (drawingWorld.workingGizmo) {
        drawingWorld.workingGizmo.dispose();
    }
    
    // Create a new utility layer
    const utilityLayer = new BABYLON.UtilityLayerRenderer(drawingWorld.scene);
    
    // Create position gizmo with the utility layer
    drawingWorld.workingGizmo = new BABYLON.PositionGizmo(utilityLayer);
    
    // IMPORTANT: Set scale BEFORE attaching
    drawingWorld.workingGizmo.scaleRatio = 3;
    
    // Attach to mesh
    drawingWorld.workingGizmo.attachedMesh = mesh;
    
    // Make sure it updates position
    drawingWorld.workingGizmo.updateGizmoRotationToMatchAttachedMesh = true;
    drawingWorld.workingGizmo.updateGizmoPositionToMatchAttachedMesh = true;
    
    
    // Force visibility
    const gizmoMeshes = utilityLayer.utilityLayerScene.meshes;
    gizmoMeshes.forEach(m => {
        m.isVisible = true;
    });
    
    return drawingWorld.workingGizmo;
};

// DEBUG ROTATION GIZMO
window.debugRotationGizmo = () => {
    ...
    const mesh = drawingWorld.scene.meshes.find(m => m.isWorkBenchPart);
    if (mesh) {
        drawingWorld.gizmoManager.attachToMesh(null); // Detach first
        drawingWorld.gizmoManager.positionGizmoEnabled = false;
        drawingWorld.gizmoManager.scaleGizmoEnabled = false;
        drawingWorld.gizmoManager.rotationGizmoEnabled = true;
        drawingWorld.gizmoManager.attachToMesh(mesh);
...
// Add debug command to check board state
window.debugBoards = function() {
    world.workBenchParts.forEach(part => {
        if (part.mesh) {
        }
    });
    
    world.scene.meshes.forEach(mesh => {
        if (mesh.isWorkBenchPart) {
        }
    });
};
