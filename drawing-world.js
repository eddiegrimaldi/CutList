...
class ModernGizmoSystem {
    ...
    setupObservables() {
        ...
        this.scene.onPointerObservable.add((pointerInfo) => {
            // Check if gizmo will handle this event first
            if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERDOWN) {
                ...
                if (pickInfo.hit && pickInfo.pickedMesh.isGizmo) {
                    // Gizmo will handle this - stop other handlers
        ...
        }, BABYLON.PointerEventTypes.POINTERDOWN | BABYLON.PointerEventTypes.POINTERMOVE | BABYLON.PointerEventTypes.POINTERUP, false, 999);

    // Part selection observer - runs regardless of sketch mode
    this.scene.onPointerObservable.add((pointerInfo) => {
        if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERDOWN) {
    ...
    }, BABYLON.PointerEventTypes.POINTERDOWN, false, 500); // Priority 500 - after gizmo but before others
...
class DrawingWorld {
                    ...
                    if (pickResult.hit && pickResult.pickedMesh === cube) {
                        ...
                        switch(faceIndex) {
                            ...
                            case 3: // BLACK side (was incorrectly marked as bottom)
                                console.log('BLACK face clicked - animating to back view');
    ...
    selectSketchTool(toolName) {
        ...
        if (toolName === 'sketch') {
            ...
            if (this.gizmoManager && this.gizmoManager.attachedMesh) {
                ...
                console.log('Switched existing gizmo to position');
    ...
    addMaterialToProject() {
        ...
        if (this.camera.mode === BABYLON.Camera.ORTHOGRAPHIC_CAMERA && this.rulerContainer) {
            ...
            // CRITICAL FIX: Set partData on the mesh so selection works!
            const mesh = this.scene.getMeshById(board.id);
            if (mesh) {
                mesh.partData = part;  // Now the mesh can be selected and manipulated
                console.log('✅ Set partData on mesh:', mesh.name, 'part:', part);
            }
...
// Simple approach: Just watch for mesh changes
        ...
        console.log('  - Position gizmo exists:', !!this.gizmoManager.gizmos.positionGizmo);
        console.log('  - Rotation gizmo exists:', !!this.gizmoManager.gizmos.rotationGizmo);
        console.log('  - Scale gizmo exists:', !!this.gizmoManager.gizmos.scaleGizmo);
...
window.testGizmoDirectly = () => {
    console.log('===== DIRECT GIZMO TEST =====');
    
    // Get the first workbench part mesh
    const mesh = drawingWorld.scene.meshes.find(m => m.isWorkBenchPart);
    if (!mesh) {
        console.log('No workbench part found to test with');
        return;
    }
    
    console.log('Testing with mesh:', mesh.name);
    ...
    console.log('GizmoManager created and attached');
    console.log('Position gizmo enabled:', drawingWorld.testGM.positionGizmoEnabled);
    console.log('Attached mesh:', drawingWorld.testGM.attachedMesh?.name);
    console.log('Position gizmo object:', drawingWorld.testGM.gizmos.positionGizmo);
...
window.testManualGizmo = () => {
    ...
    console.log('Manual PositionGizmo created and attached');
    console.log('Gizmo:', drawingWorld.manualGizmo);
    console.log('Attached to:', mesh.name);
...
window.testGizmoWithLayer = () => {
    console.log('===== GIZMO WITH UTILITY LAYER TEST =====');
    ...
    if (!drawingWorld.utilityLayer) {
        ...
        console.log('Created utility layer');
    }
    ...
    console.log('PositionGizmo with utility layer created');
    console.log('Gizmo:', drawingWorld.testGizmoWL);
    console.log('Utility layer:', drawingWorld.utilityLayer);
    console.log('Attached to:', mesh.name);
    
    // Also check visibility
    console.log('Gizmo root mesh:', drawingWorld.testGizmoWL._rootMesh);
    if (drawingWorld.testGizmoWL._rootMesh) {
        console.log('Root mesh visible:', drawingWorld.testGizmoWL._rootMesh.isVisible);
        console.log('Root mesh enabled:', drawingWorld.testGizmoWL._rootMesh.isEnabled());
    }
...
window.createWorkingGizmo = () => {
    console.log('===== CREATING WORKING GIZMO =====');
    
    const mesh = drawingWorld.scene.meshes.find(m => m.isWorkBenchPart);
    if (!mesh) {
        console.log('No workbench part found');
        return;
    }
    ...
    console.log('Created working gizmo');
    console.log('Attached to:', mesh.name);
    console.log('Position:', mesh.position);
    ...
    gizmoMeshes.forEach(m => {
        m.isVisible = true;
        console.log('Gizmo mesh:', m.name, 'visible:', m.isVisible);
    });
...
window.debugRotationGizmo = () => {
    console.log('===== ROTATION GIZMO DEBUG =====');
    
    if (!drawingWorld.gizmoManager) {
        console.log('No gizmoManager exists');
        return;
    }
    
    console.log('GizmoManager:', drawingWorld.gizmoManager);
    console.log('Rotation enabled:', drawingWorld.gizmoManager.rotationGizmoEnabled);
    console.log('Position enabled:', drawingWorld.gizmoManager.positionGizmoEnabled);
    console.log('Scale enabled:', drawingWorld.gizmoManager.scaleGizmoEnabled);
    ...
    if (drawingWorld.gizmoManager.gizmos.rotationGizmo) {
        const rg = drawingWorld.gizmoManager.gizmos.rotationGizmo;
        console.log('Rotation gizmo exists!');
        console.log('Attached mesh:', rg.attachedMesh?.name);
        console.log('Attached node:', rg.attachedNode?.name);
        console.log('Scale ratio:', rg.scaleRatio);
        
        // Check individual axis gizmos
        console.log('X gizmo:', rg.xGizmo);
        console.log('Y gizmo:', rg.yGizmo);
        console.log('Z gizmo:', rg.zGizmo);
        
        if (rg.xGizmo) {
            console.log('X gizmo isEnabled:', rg.xGizmo.isEnabled);
            console.log('X gizmo attachedMesh:', rg.xGizmo.attachedMesh?.name);
        }
    } else {
        console.log('No rotation gizmo created!');
    }
    
    // Try to manually create one
    console.log('\nAttempting manual rotation gizmo creation...');
    const mesh = drawingWorld.scene.meshes.find(m => m.isWorkBenchPart);
    if (mesh) {
