...
class DrawingWorld {
    ...
    addMaterialToProject() {
        ...
        console.log('Material added to work bench:', part);
    }
...
// Create material with wood texture        box.material = this.createMaterialWithTexture(part.materialId, part.materialName);
...
n    // Create material with actual wood texture - FIXED VERSION
    createMaterialWithTexture(materialId, materialName) {
        const material = new BABYLON.StandardMaterial(`${materialId}_material_${Date.now()}`, this.scene);
        
        console.log(`Creating textured material for ${materialName} (${materialId})`);
        
        // Try to get texture path from materials library
        let texturePath = null;
        if (this.materialsLibrary && this.materialsLibrary.isLoaded) {
            const texturePaths = this.materialsLibrary.getTexturePaths(materialId);
            if (texturePaths && texturePaths.texture_diffuse) {
                texturePath = texturePaths.texture_diffuse;
        ...
        if (texturePath) {
            try {
                // Load the actual wood texture
                const texture = new BABYLON.Texture(texturePath, this.scene);
                material.diffuseTexture = texture;
                
                // Set texture properties for better wood appearance
                texture.wrapU = BABYLON.Texture.WRAP_ADDRESSMODE;
                texture.wrapV = BABYLON.Texture.WRAP_ADDRESSMODE;
                texture.uScale = 2.0; // Repeat texture for better grain
                texture.vScale = 2.0;
                
                // Add some material properties for realistic wood
                material.specularColor = new BABYLON.Color3(0.1, 0.1, 0.1); // Low specularity for wood
                material.specularPower = 32;
                
                console.log(`Successfully loaded texture for ${materialName}`);
                
            } catch (error) {
                console.error(`Error loading texture ${texturePath}:`, error);
                // Fallback to color
                material.diffuseColor = this.getMaterialColor(materialId);
            }
        } else {
            console.log(`No texture path found for ${materialId}, using color fallback`);
...
// Create material with wood texture for new boxes            box.material = this.createMaterialWithTexture(part.materialId, part.materialName);
...
// Create material with wood texture for restored parts without material            box.material = this.createMaterialWithTexture(part.materialId, part.materialName);
        }
    ...
    createAssemblyProjectView() {
        ...
        this.projectParts.forEach((part, index) => {
            ...
            if (!box) {
                ...
                // Set position if saved geometry exists
            ...
            if (!box.material) {
                const material = new BABYLON.StandardMaterial(`${part.id}_material`, this.scene);
                // Create material with wood texture for restored assembly parts without material
                box.material = this.createMaterialWithTexture(part.materialId, part.materialName);
            }
            // Store reference
            box.partData = part;
            box.isProjectPart = true;
