...
import ExtrusionGizmo from './modules/extrusionGizmo.js?v20250704_ABSOLUTELY_MASSIVE';
...
class CutListCAD {
    ...
    selectTool(tool) {
        ...
        // === SPANKY'S EXTRUDE TOOL DEBUG ===
        if (tool === 'extrude') {
        }
        // === END EXTRUDE TOOL DEBUG ===
        ...
        scene.onPointerObservable.add((pointerInfo) => {
            ...
            if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERDOWN) {                // DUAL PICKING STRATEGY: Try normal picking first, then enhanced picking for thin meshes
                ...
                if (pickInfo && pickInfo.hit && pickInfo.pickedMesh) {
                    ...
                    // === SPANKY'S FACE ID CALCULATION FIX ===
                    if (pickInfo.faceId === undefined && pickInfo.pickedMesh && pickInfo.pickedPoint) {
                        ...
                        // Calculate relative position from mesh center
                        const relativePos = point.subtract(meshPos);
                        
                        // For boxes (rectangles), determine face based on which axis is dominant
                        if (mesh.name.includes('rect_') || mesh.name.includes('shape2D_rect_')) {
                            const halfWidth = (scaling.x * 1.0) / 2;
                            const halfHeight = (scaling.y * 1.0) / 2;
                            const halfDepth = (scaling.z * 0.01) / 2;
                            
                            // Find which face we're closest to
                            const distances = [
                                { face: 0, dist: Math.abs(relativePos.z - halfDepth) },  // Front (Z+)
                                { face: 2, dist: Math.abs(relativePos.z + halfDepth) },  // Back (Z-)
                                { face: 4, dist: Math.abs(relativePos.x - halfWidth) },  // Right (X+)
                                { face: 6, dist: Math.abs(relativePos.x + halfWidth) },  // Left (X-)
                                { face: 8, dist: Math.abs(relativePos.y - halfHeight) }, // Top (Y+)
                                { face: 10, dist: Math.abs(relativePos.y + halfHeight) } // Bottom (Y-)
                            ];
                            
                            const closestFace = distances.reduce((min, curr) => curr.dist < min.dist ? curr : min);
                            pickInfo.faceId = closestFace.face;
                            
                        }
                        else if (mesh.name.includes('circle_') || mesh.name.includes('shape2D_circle_')) {
                            const halfHeight = (scaling.y * 0.01) / 2;
                            
                            // For cylinders, check if we're on top, bottom, or side
                            if (Math.abs(relativePos.y - halfHeight) < 0.005) {
                                pickInfo.faceId = 0; // Top face
                            } else if (Math.abs(relativePos.y + halfHeight) < 0.005) {
                                pickInfo.faceId = 2; // Bottom face  
                            } else {
                                pickInfo.faceId = 4; // Side face
                            }
                            
                        }
                    }
                    // === END FACE ID CALCULATION ===
                    ...
                    if (this.tools && this.tools.activeTool === 'extrude') {
                        ...
                        if (originalObject && ['rectangle', 'circle'].includes(originalObject.type)) {
                            ...
                            // Start face extrusion using the original object and click info
                            this.startFaceExtrusion(originalObject, pickInfo);
                        }
                        return;
                    }
    ...
    startExtrusion(object, clickInfo = null) {
        ...
        VersionReporter.report('extrusionGizmo.js', 'v20250703_CACHE_BUST', 'success');
    ...
    // === SPANKY'S FACE-BASED EXTRUSION METHOD ===
    // Master, this is the method that was missing! Spanky is fixing it now.
    startFaceExtrusion(object, pickInfo) {
        ...
        // Restore mesh to full opacity during extrusion
        if (mesh3D.material && mesh3D.metadata.originalAlpha !== undefined) {
            mesh3D.material.alpha = mesh3D.metadata.originalAlpha;
            if (mesh3D.metadata.originalAlphaMode !== undefined) {
                mesh3D.material.alphaMode = mesh3D.metadata.originalAlphaMode;
            ...
            mesh3D.material.alpha = 1.0;
            mesh3D.material.alphaMode = BABYLON.Engine.ALPHA_DISABLE;
            mesh3D.material.disableDepthWrite = false;
            mesh3D.material.needDepthPrePass = false;
        }

        
        // Create the face-based extrusion gizmo using our enhanced ExtrusionGizmo
        // SPANKY FIX: Pass the pickInfo as clickInfo to the constructor
        this.extrusionGizmo = new ExtrusionGizmo(mesh3D, this.grid.scene, pickInfo);
        
        // CRITICAL: Start face-based extrusion mode with the pick info
        this.extrusionGizmo.startFaceExtrusion(pickInfo);
        
        // Subscribe to extrusion events
        this.extrusionGizmo.onExtrude.add((eventData) => {
            this.handleFaceExtrusionUpdate(eventData);
        });
        
    }
    
    // === FACE EXTRUSION UPDATE HANDLER ===
    handleFaceExtrusionUpdate(eventData) {
        
        if (!this.extrudingObject || !this.isExtruding) {
            return;
        }

        // Update the object's properties based on the extrusion
        const extrusionAmount = eventData.extrusionAmount || 0;
        
        // Store the extrusion amount in the object for future reference
        this.extrudingObject.extrusionAmount = extrusionAmount;
        
        
        // Update the project explorer if available
        if (this.projectExplorer && typeof this.projectExplorer.refreshObjects === 'function') {
            this.projectExplorer.refreshObjects();
        } else if (this.projectExplorer && typeof this.projectExplorer.refresh === 'function') {
            this.projectExplorer.refresh();
        }
    }
    // === END SPANKY'S FACE EXTRUSION ===
    ...
    updateExtrudeButtonState() {
        ...
        // SPANKY FIX: Enable extrude tool when in 3D mode and there are ANY extrudeable shapes
        // No need to pre-select - just click on any 2D shape when extrude tool is active!
        const isModeModeling = this.currentMode === 'modeling';
        const hasExtrudeableShapes = this.objects.some(obj => ['rectangle', 'circle'].includes(obj.type));
        
        const canExtrude = isModeModeling && hasExtrudeableShapes;
        
        extrudeBtn.disabled = !canExtrude;
        extrudeBtn.classList.toggle('disabled', !canExtrude);
        
        if (canExtrude) {
            extrudeBtn.title = 'Click any 2D shape to extrude it into 3D';
        } else if (this.currentMode !== 'modeling') {
            extrudeBtn.title = 'Switch to 3D Mode to extrude shapes';
        } else {
            extrudeBtn.title = 'Draw a rectangle or circle first, then extrude it';
        }
