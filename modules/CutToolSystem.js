...
export class CutToolSystem {
    ...
    executeCut(mesh, cutPosition) {
        ...
        if (!mesh1) {
            console.error("Failed to create mesh1 for piece:", piece1Data);
            return;
        }
        ...
        // SIMPLE CORNER-BASED POSITIONING: Keep pieces in original footprint
        // The cut happens at the blade position, pieces stay in place separated by kerf
        const halfKerf = kerfWidth / 2;
        
        // Get the original board's bounds
        const originalBounds = mesh.getBoundingInfo();
        const minPos = originalBounds.minimum;
        const maxPos = originalBounds.maximum;
        
        if (cutAxis === 'z') {
            // Cross cut - blade moves along Z (length), pieces split along X
            // Blade position is in local coordinates, we need world position
            const bladeLocalPos = this.currentCutLine.position.x; // Blade's X position in local space
            const bladeWorldX = originalPosition.x + bladeLocalPos; // Convert to world
            
            // Piece 1 gets left side of cut
            mesh1.position.x = (minPos.x + (bladeWorldX - halfKerf)) / 2;
        ...
        if (!mesh2) {
            console.error("Failed to create mesh2 for piece:", piece2Data);
            return;
        }
        ...
        // SIMPLE CORNER-BASED POSITIONING: Piece 2 stays in place too
        if (cutAxis === 'z') {
            // Cross cut - blade moves along Z, pieces split along X
            // Piece 2 gets the right portion of the original board
            const bladeLocalPos = this.currentCutLine.position.x;
            const bladeWorldX = originalPosition.x + bladeLocalPos;
            mesh2.position.x = ((bladeWorldX + halfKerf) + maxPos.x) / 2;
            mesh2.position.y = originalPosition.y;
            mesh2.position.z = originalPosition.z;
        } else if (cutAxis === 'x') {
            // Rip cut - blade moves along X, pieces split along Z
            // Piece 2 gets the back portion of the original board
            const bladeLocalPos = this.currentCutLine.position.z;
            const bladeWorldZ = originalPosition.z + bladeLocalPos;
            mesh2.position.x = originalPosition.x;
            mesh2.position.y = originalPosition.y;
            mesh2.position.z = ((bladeWorldZ + halfKerf) + maxPos.z) / 2;
        } else if (cutAxis === 'y') {
            mesh2.position.x = originalPosition.x;
            const cutWorldY = this.currentCutLine.position.y;
            mesh2.position.y = ((cutWorldY + halfKerf) + maxPos.y) / 2;
            mesh2.position.z = originalPosition.z;
        } else {
            mesh2.position.x = originalPosition.x;
            mesh2.position.y = originalPosition.y;
    ...
    animateCameraForCutting(cutDirection) {
        ...
        // If selectedPart is a Part object, find its mesh by ID
        if (targetMesh && targetMesh.constructor && targetMesh.constructor.name === "Part") {
            const partId = targetMesh.id;
            // Find the mesh with this part ID
            targetMesh = this.scene.getMeshByName(partId);
            if (!targetMesh) {
                // Try to find by partInstance
                targetMesh = this.scene.meshes.find(m => m.partInstance && m.partInstance.id === partId);
            }
        }
        
        // If it's just part data, find the mesh
        if (targetMesh && !targetMesh.getBoundingInfo && targetMesh.id) {
            const partId = targetMesh.id;
            targetMesh = this.scene.getMeshByName(partId);
            if (!targetMesh) {
                targetMesh = this.scene.meshes.find(m => m.partInstance && m.partInstance.id === partId);
