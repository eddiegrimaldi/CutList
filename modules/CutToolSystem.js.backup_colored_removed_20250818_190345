...
//         this.createColoredPreviewPieces(mesh, cutLine);
    ...
    executeCut(mesh, cutPosition) {
        ...
        // Calculate the kerf offset (half kerf on each side)
        
        if (cutAxis === 'x') {
            // For X-axis cut: piece 1 gets left side, piece 2 gets right side
            const piece1CenterX = (minPos.x + actualCutPos - kerfOffsetCm) / 2;
            mesh1.position.x = piece1CenterX;
            mesh1.position.y = originalPosition.y;
            mesh1.position.z = originalPosition.z;
        } else if (cutAxis === 'y') {
            // For Y-axis cut: piece 1 gets bottom side, piece 2 gets top side
            const piece1CenterY = (minPos.y + actualCutPos - kerfOffsetCm) / 2;
            mesh1.position.x = originalPosition.x;
            mesh1.position.y = piece1CenterY;
            mesh1.position.z = originalPosition.z;
        } else {
            // For Z-axis cut: piece 1 gets front side, piece 2 gets back side
            const piece1CenterZ = (minPos.z + actualCutPos - kerfOffsetCm) / 2;
            mesh1.position.x = originalPosition.x;
            mesh1.position.y = originalPosition.y;
            mesh1.position.z = piece1CenterZ;
        }
        ...
        if (cutAxis === 'x') {
            // For X-axis cut: piece 2 gets right side
            const piece2CenterX = (actualCutPos + kerfOffsetCm + maxPos.x) / 2;
            mesh2.position.x = piece2CenterX;
            mesh2.position.y = originalPosition.y;
            mesh2.position.z = originalPosition.z;
        } else if (cutAxis === 'y') {
            // For Y-axis cut: piece 2 gets top side
            const piece2CenterY = (actualCutPos + kerfOffsetCm + maxPos.y) / 2;
            mesh2.position.x = originalPosition.x;
            mesh2.position.y = piece2CenterY;
            mesh2.position.z = originalPosition.z;
        } else {
            // For Z-axis cut: piece 2 gets back side
            const piece2CenterZ = (actualCutPos + kerfOffsetCm + maxPos.z) / 2;
            mesh2.position.x = originalPosition.x;
            mesh2.position.y = originalPosition.y;
            mesh2.position.z = piece2CenterZ;
        }
