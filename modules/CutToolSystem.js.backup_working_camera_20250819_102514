...
export class CutToolSystem {
    ...
    onMouseMove(pointerInfo) {
        ...
        // If we have a hovered part from before, keep tracking it
        // If we have a hovered part from before, keep tracking it        if (this.hoveredPart) {            const mesh = this.hoveredPart;            const partData = this.getPartData(mesh);                        // Create a ray from the camera through the mouse position            const ray = this.scene.createPickingRay(                this.scene.pointerX,                this.scene.pointerY,                BABYLON.Matrix.Identity(),                this.drawingWorld.camera            );                        // Get the board's world matrix to understand its orientation            const worldMatrix = mesh.getWorldMatrix();            const boardPos = mesh.getAbsolutePosition();                        // For cross-cuts, we want movement along the board's length (local Z)            // For rip-cuts, we want movement along the board's width (local X)            let planeNormal;                        if (this.activeCutDirection === 'cross') {                // Cross-cut: blade moves along length, so we need a plane perpendicular to width                // The plane should contain the length axis and be visible to the camera                planeNormal = BABYLON.Vector3.TransformNormal(                    new BABYLON.Vector3(1, 0, 0),  // Local X axis (width direction)                    worldMatrix                );            } else {                // Rip-cut: blade moves along width, so we need a plane perpendicular to length                planeNormal = BABYLON.Vector3.TransformNormal(                    new BABYLON.Vector3(0, 0, 1),  // Local Z axis (length direction)                    worldMatrix                );            }                        // Make sure the plane faces the camera (dot product with ray direction)            if (BABYLON.Vector3.Dot(planeNormal, ray.direction) > 0) {                planeNormal = planeNormal.scale(-1);            }                        // Create the plane at the board's position            const plane = BABYLON.Plane.FromPositionAndNormal(boardPos, planeNormal);                        // Find where the ray intersects the plane            const distance = ray.intersectsPlane(plane);            if (distance && distance > 0) {                const hitPoint = ray.origin.add(ray.direction.scale(distance));                                // Create a fake pickInfo with the calculated hit point                const fakePickInfo = {                    hit: true,                    pickedMesh: mesh,                    pickedPoint: hitPoint,                    distance: distance                };                                // Update blade position with the projected point                this.updateCutPosition(mesh, partData, fakePickInfo);            }            return;        }        }
        
        // Original picking logic for when not tracking a board
    ...
    createCutPreview(mesh, partData, pickInfo) {
        ...
        this.cutPreviewLine.position.y += 0.05;
    ...
    updateCutPosition(mesh, partData, pickInfo) {
        ...
        this.cutPreviewLine.position = cutLine.position.clone();
        this.cutPreviewLine.position.y += 0.05; // Keep offset to avoid z-fighting
    ...
    calculateCutLine(mesh, dimensions, pickInfo) {
        ...
        // For cross cuts, blade moves along the board's length        // Don't try to determine world axis - just use local coordinates        if (this.activeCutDirection === 'cross') {            cutAxis = 'z';  // Cross cuts always use Z axis        } else {            cutAxis = 'x';  // Rip cuts use X axis        }        // For rotated boards, don't try to match world axes - use local axes directly        if (this.activeCutDirection === 'cross') {            cutAxis = 'z';  // Cross cuts always move along local Z (length)        } else {            cutAxis = 'x';  // Rip cuts always move along local X (width)        }        }
        ...
        if (cutAxis === 'x') {            // Rip cut - blade moves along the board's width (local X axis)            const boardWidth = dimensions.width_inches * 2.54;            const boardThickness = dimensions.thickness_inches * 2.54;            const boardLength = dimensions.length_inches * 2.54;                        cutLine.width = 0.3;  // Thin blade            cutLine.height = boardThickness + 10;  // Blade height above board            cutLine.depth = boardLength + 1;  // Blade spans the length                        // Use LOCAL coordinates since blade is parented to board            const localX = Math.max(-boardWidth/2, Math.min(boardWidth/2, localHitPoint.x));                        // Position in LOCAL coordinates (blade is parented to board)            cutLine.position = new BABYLON.Vector3(                localX,  // Position along width (local X)                boardThickness/2 + 5,  // Above the board (local Y)                0  // Center on length (local Z)            );                        // Normalized position along the board (0 to 1)            cutLine.normalizedPosition = (localX + boardWidth/2) / boardWidth;                    if (cutAxis === 'x') {
            cutLine.width = 0.3;
            ...
            // Cross cut - blade moves along the board's length (local Z axis)
            const boardWidth = dimensions.width_inches * 2.54;
            const boardThickness = dimensions.thickness_inches * 2.54;
            const boardLength = dimensions.length_inches * 2.54;
            
            cutLine.width = boardWidth + 1;  // Blade spans the width
            cutLine.height = 5;  // Fixed blade height
            cutLine.depth = 0.3;  // Thin blade
            
            // Use LOCAL coordinates since blade is parented to board
            // Local Z ranges from -boardLength/2 to +boardLength/2
            const localZ = Math.max(-boardLength/2, Math.min(boardLength/2, localHitPoint.z));
            
            // Position in LOCAL coordinates (blade is parented to board)
            cutLine.position = new BABYLON.Vector3(
                0,  // Center on width (local X)
                boardThickness/2 + 5,  // Above the board (local Y)
                localZ  // Position along length (local Z)
            );
            console.log("Blade position (local):", cutLine.position, "localZ:", localZ);
            
            // Normalized position along the board (0 to 1)
            cutLine.normalizedPosition = (localZ + boardLength/2) / boardLength;
        }
        // Store cut info
