...
export class RouterBitSystem {
    ...
    createCuttingToolGeometry(edge, profileName, size) {
        ...
        if (profileName === 'roundover') {
            // Create a torus for proper roundover - when subtracted leaves convex rounded edge
            cuttingTool = BABYLON.TorusBuilder.CreateTorus('roundover_cutter', {
                diameter: sizeInCm * 2, // Major diameter
                thickness: sizeInCm, // Minor diameter (thickness of torus)
                tessellation: 16
            }, this.scene);
    ...
    positionCuttingTool(cuttingTool, edge, meshSize, sizeInCm) {
        ...
        if (edge.type === 'top_front_edge') {
            ...
            if (toolName.includes('roundover')) {
                // Position sphere to cut corner - offset so only 1/4 of sphere intersects board
                cuttingTool.position.y += (meshSize.y / 2) - (sizeInCm / 2); // Half sphere radius into top surface
                cuttingTool.position.z += (meshSize.z / 2) - (sizeInCm / 2); // Half sphere radius into front surface
            ...
            if (toolName.includes('roundover')) {
                cuttingTool.position.y += (meshSize.y / 2) - (sizeInCm / 2); // Half sphere radius into top surface
                cuttingTool.position.z -= (meshSize.z / 2) - (sizeInCm / 2); // Half sphere radius into back surface
            } else if (toolName.includes('chamfer')) {
                // Position rectangular box for 45-degree chamfer - intersect at corner
                cuttingTool.position.y += (meshSize.y / 2); // At top surface level
            ...
            if (toolName.includes('roundover')) {
                cuttingTool.position.y += (meshSize.y / 2) - (sizeInCm / 2); // Half sphere radius into top surface
                cuttingTool.position.x -= (meshSize.x / 2) - (sizeInCm / 2); // Half sphere radius into left surface
            } else if (toolName.includes('chamfer')) {
                // Position rectangular box for 45-degree chamfer - intersect at corner
                cuttingTool.position.y += (meshSize.y / 2); // At top surface level
            ...
            if (toolName.includes('roundover')) {
                cuttingTool.position.y += (meshSize.y / 2) - (sizeInCm / 2); // Half sphere radius into top surface
                cuttingTool.position.x += (meshSize.x / 2) - (sizeInCm / 2); // Half sphere radius into right surface
            } else if (toolName.includes('chamfer')) {
                // Position rectangular box for 45-degree chamfer - intersect at corner
                cuttingTool.position.y += (meshSize.y / 2); // At top surface level
    ...
    performCSGSubtraction(boardMesh, cuttingTool, profileName) {
        ...
        try {
            ...
            if (cuttingTool.material) {
                cuttingTool.material.alpha = 0.3;
                cuttingTool.material.emissiveColor = new BABYLON.Color3(1, 0, 0);
            } else {
                const debugMat = new BABYLON.StandardMaterial('debug', this.scene);
                debugMat.alpha = 0.3;
            ...
            // Clean up cutting tool after successful CSG operation
            cuttingTool.dispose();
            
            // CRITICAL: Also remove any lingering references to prevent reappearance
            if (this.scene.getMeshByName(cuttingTool.name)) {
                this.scene.removeMesh(cuttingTool);
            }
