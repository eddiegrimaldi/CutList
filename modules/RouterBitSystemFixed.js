...
export class RouterBitSystem {
    ...
    recreateMeshFromJSON(bitData) {
        try {
            ...
            // Protect mesh from disposal
            const originalDispose = mesh.dispose.bind(mesh);
            mesh.dispose = function() {
                // Don't actually dispose the imported mesh
            };
            mesh._protected = true;
    ...
    showBitSizeSelector(bitProfile) {
        if (this.routerBitGrid && this.bitSizeSelector) {
            ...
            if (this.selectedBitName) {
                const bitName = bitProfile && bitProfile.name ? bitProfile.name : 'Imported Router Bit';
                this.selectedBitName.textContent = `${bitName} - Select Size`;
            }
    ...
    populateRouterBitGrid() {
        ...
        Object.entries(this.routerBitLibrary).forEach(([key, bitProfile]) => {
            ...
            const safeName = bitProfile && bitProfile.name ? bitProfile.name : 'Imported Router Bit';
            const safeDescription = bitProfile && bitProfile.description ? bitProfile.description : 'Custom imported router bit';
            const safeIcon = bitProfile && bitProfile.icon ? bitProfile.icon : '🔧';
            
            bitCard.innerHTML = `
                <span class="router-bit-icon">${safeIcon}</span>
                <div class="router-bit-name">${safeName}</div>
                <div class="router-bit-description">${safeDescription}</div>
            `;
    ...
    populateSizeButtons(bitProfile) {
        ...
        // Handle imported router bits that don't have bitProfile
        const defaultSizes = (bitProfile && bitProfile.defaultSizes) ? bitProfile.defaultSizes : [0.25, 0.5, 0.75, 1.0];
    ...
    applyProfileToSelectedEdges() {
        ...
        // OVERRIDE: Use imported router bit if available instead of selectedBitProfile
        let profileToUse = this.selectedBitProfile;
        if (this.customRouterBits.size > 0) {
            const firstImported = Array.from(this.customRouterBits.keys())[0];
            profileToUse = firstImported;
        }
        ...
        const edgeCount = this.selectedEdges.length;
        this.selectedEdges.forEach(edge => {
            this.applyProfileToEdge(edge, profileToUse, this.currentBitSize);
        });
    ...
    createCuttingToolFromImportedBit(edge, importedBit, scale) {
        ...
        // For imported bits: already scaled 5000x during import, only apply user scale
        // For built-in bits: need full scaling since they're created at normal size
        const finalScale = isImported ? scale : scale * 5000;
        cuttingTool.scaling.setAll(finalScale);
        
        // CORNER TO CORNER POSITIONING - PUT ROUTER BIT CORNER AT BOARD CORNER
        const edgeCornerPosition = this.getEdgeCornerPosition(edge);
        console.log('Edge type:', edge.type, 'Corner at:', edgeCornerPosition);
        
        // Position tool for cutting
        cuttingTool.position.copyFrom(edgeCornerPosition);
        console.log('Router bit positioned at:', cuttingTool.position);
        
        // Router bit now positioned at exact corner - no additional offsets needed
    ...
    createUniversalCuttingTool(edge, routerBitMesh, scale, isImported) {
        ...
        if (isImported) {
            // For imported bits: scale the actual vertex data for CSG operations
            // PRECISE CALCULATION based on actual dimensions:
            // Board: 15.2 x 1.9 x 243.8 units (from logs: mesh bounds)
            // Original router bit: 0.002400 x 0.001000 x 0.002970 units 
            // Target: 1/4" router bit = 0.635 cm diameter
            // Board width 6" = 15.24 cm, so 1 unit = 1 cm
            // Therefore: scale = 0.635 / 0.002400 = 264.6, but that's too big
            // Let's use the board thickness (1.9 units = 0.75") as reference
            // 0.75" = 1.905 cm, so 1 unit = 1.27 cm  
            // Scale = (0.635 cm) / (0.002400 * 1.27 cm/unit) = 208
            // Still too big. Let's try empirical: we want ~1/20 of board width
            // 15.2 / 20 = 0.76 units target diameter
            // Scale = 0.76 / 0.002400 = 317... still huge
            // Back to empirical testing but with reasoning:
            const imperialScale = 0.1; // Will make bit ~0.00024 units = very small
            
            // Get vertex data and scale it directly
            const positions = cuttingTool.getVerticesData(BABYLON.VertexBuffer.PositionKind);
            if (positions) {
                for (let i = 0; i < positions.length; i++) {
                    positions[i] *= imperialScale;
                }
                cuttingTool.setVerticesData(BABYLON.VertexBuffer.PositionKind, positions);
            }
        ...
        // This positioning is handled later in the method - removing duplicate
        
        // Apply proper orientation for the edge type
        // For top_left_edge, router bit should be upright (no rotation needed)
        cuttingTool.rotation.x = 0;
        cuttingTool.rotation.y = 0; 
        cuttingTool.rotation.z = 0;
        
        // Make cutting tool visible for inspection
        cuttingTool.material = new BABYLON.StandardMaterial('cutting_tool_material', this.scene);
        cuttingTool.material.diffuseColor = new BABYLON.Color3(1, 0, 0); // Bright red
        cuttingTool.material.alpha = 0.7; // Semi-transparent
        cuttingTool.setEnabled(true);
        ...
        cuttingTool.refreshBoundingInfo();
        cuttingTool.computeWorldMatrix(true);
        
        const boundingInfo = cuttingTool.getBoundingInfo();
        const size = boundingInfo.maximum.subtract(boundingInfo.minimum);
        
        // Size validation removed - let's see the router bit regardless of size
        ...
        switch (edge.type) {
            ...
            case 'top_right_edge':
                return new BABYLON.Vector3(
                    ...
                    bounds.maximum.y,  // Top face
                    bounds.maximum.z   // Front corner of right edge
                );
            default:
    ...
    createLegacyChamferTool(edge, profileName, size) {
        ...
        if (cuttingTool) {
            // UNIVERSAL CORNER ALIGNMENT - positioning handled by createUniversalCuttingTool
            // Legacy positionCuttingTool() disabled - caused wrong positioning
        }
    ...
    performCSGSubtraction(boardMesh, cuttingTool, profileName) {
        ...
        try {
            ...
            // SKIP CSG - POSITION ISSUE CAUSES EMPTY RESULT
            return; // Skip CSG for now
            ...
            // Keep the cutting tool visible for inspection - DON'T dispose
            if (cuttingTool) {
                // cuttingTool.dispose();  // COMMENTED OUT for visibility
            }
