...
class TheMillSystem {
    ...
    createSmartToolbar() {
        ...
        smartToolbar.style.height = '100px';
        ...
        const miterControl = this.createSliderControl('Miter', -90, 90, 0, (value) => {
            this.bladeAngle = value * Math.PI / 180;
            if (this.blade) this.blade.rotation.y = this.bladeAngle;
            if (this.laserLine) this.laserLine.rotation.y = this.bladeAngle;
            // Update split camera position to match blade angle
            if (this.splitScreenEnabled) {
                this.updateSplitScreen(true);
            }
        });
        smartToolbar.appendChild(miterControl);
        
        // Bevel angle control
        const bevelControl = this.createSliderControl('Bevel', -45, 45, 0, (value) => {
            this.bevelAngle = 90 - Math.abs(value);
            this.bevelDirection = value < 0 ? -1 : 1;
        ...
        resetButton.onclick = () => {
            ...
            // Reset sliders
            const miterSlider = document.querySelector('#miter-slider');
            const bevelSlider = document.querySelector('#bevel-slider');
            if (miterSlider) {
                miterSlider.value = 0;
                miterSlider.dispatchEvent(new Event('input'));
            }
            if (bevelSlider) {
                bevelSlider.value = 0;
                bevelSlider.dispatchEvent(new Event('input'));
            }
            this.disableSplitScreen();
        };
        ...
        // Adjust canvas container position
        const canvasContainer = document.getElementById('mill-container');
        if (canvasContainer) {
            canvasContainer.style.top = '160px';
        }
    }
    
    createToggleButton(label, initialState, onChange) {
        ...
        button.style.background = initialState ? '#4CAF50' : '#666';
        ...
        button.onclick = () => {
            state = !state;
            button.style.background = state ? '#4CAF50' : '#666';
            onChange(state);
        };
    ...
    createSliderControl(label, min, max, defaultValue, onChange) {
        ...
        slider.type = 'range';
        slider.id = label.toLowerCase() + '-slider';
        slider.min = min;
        slider.max = max;
        ...
        presets.style.gap = '2px';
        presets.style.marginTop = '5px';
        
        const presetValues = label === 'Miter' ? [-45, -22.5, 0, 22.5, 45] : [-45, -22.5, 0, 22.5, 45];
    ...
    enableSplitScreen() {
        ...
        label.textContent = 'BLADE-EYE VIEW (ORTHO)';
    ...
    updateSplitScreen(shouldShow) {
        if (shouldShow && !this.splitScreenEnabled) {
            this.enableSplitScreen();
        } else if (!shouldShow && this.splitScreenEnabled) {
            this.disableSplitScreen();
        }
        
        // Update camera position if split screen is active
        if (this.splitScreenEnabled && this.splitCamera) {
            const bladeAngle = this.bladeAngle || 0;
            const bevelRad = ((90 - (this.bevelAngle || 90)) * Math.PI) / 180;
            
            // Update blade-eye camera position based on current angles
            this.splitCamera.position = new BABYLON.Vector3(
                Math.sin(bladeAngle * Math.PI / 180) * 200,
                Math.sin(bevelRad) * 50,
                Math.cos(bladeAngle * Math.PI / 180) * 200
            );
    ...
    updateBladeEyeCamera() {
        ...
        const miterAngle = this.blade.rotation.y || 0;
        
        // Blade is 500 wide, get the ACTUAL end position
        const halfWidth = 250;
        const rightEndX = bladePos.x + halfWidth * Math.cos(miterAngle);
        const rightEndZ = bladePos.z - halfWidth * Math.sin(miterAngle);  // Negative because miterAngle is negative
    ...
    createMillInterface() {
        ...
        canvasContainer.style.top = '180px';  // Below header and toolbar
    ...
    setupMillScene() {
        ...
        this.millEngine = new BABYLON.Engine(this.millCanvas, true);
        ...
        bladeMat.diffuseColor = new BABYLON.Color3(1, 0, 0);  // RED for visibility
        ...
        window.addEventListener('keydown', (e) => {
            if (e.key === 't' || e.key === 'T') {
                ...
                // UNLOCK camera rotation limits from blade-eye view
                this.millCamera.lowerAlphaLimit = null;
                this.millCamera.upperAlphaLimit = null;
                this.millCamera.lowerBetaLimit = 0;
                this.millCamera.upperBetaLimit = Math.PI / 2;
                
                // Reset blade thickness if it was scaled in B view
                if (this.blade) {
                    this.blade.scaling.z = 1;
                }
                ...
                if (this.currentBoard) {
                    ...
                    console.log('T key framing - bounds:', {minX, maxX, minZ, maxZ, maxSize, paddedSize});
                    const aspectRatio = this.millCanvas.width / this.millCanvas.height;
        ...
        // Make blade visible in this view
        if (this.blade) {
            this.blade.isVisible = true;
            this.blade.material.alpha = 0.7;  // Semi-transparent to see through
            console.log("Blade made visible for blade-eye view");
        }
        
        // Make board visible too
        if (this.currentBoard) {
            this.currentBoard.isVisible = true;
        }
        
        // Show table surface for reference
        if (this.tableSurface) {
            this.tableSurface.isVisible = true;
                ...
                // Isometric view - perfect for seeing bevels
                this.millCamera.mode = BABYLON.Camera.ORTHOGRAPHIC_CAMERA;
                ...
                // Reset blade thickness
                if (this.blade) {
                    this.blade.scaling.z = 1;
                }
                
                // Set to isometric angle (45 degrees around, 35 degrees up)
                this.millCamera.alpha = -Math.PI / 4;  // 45 degrees
                this.millCamera.beta = Math.PI / 3;    // 60 degrees from vertical (30 degrees up)
                
                // Frame the pieces or board
                if (this.cutPieces && this.cutPieces.length > 0) {
                    ...
                    const sizeZ = maxZ - minZ;
                    const maxSize = Math.max(sizeX, sizeZ);
                    const paddedSize = maxSize * 1.5;  // More padding for iso view
                    const aspectRatio = this.millCanvas.width / this.millCanvas.height;
                    this.millCamera.orthoLeft = -paddedSize * aspectRatio / 2;
                    this.millCamera.orthoRight = paddedSize * aspectRatio / 2;
                    this.millCamera.orthoTop = paddedSize / 2;
                ...
                console.log('Switched to isometric view - press I to see bevels clearly');
            }
        });
        ...
        this.updateBladeRotation = () => {
            if (this.blade) {
                ...
                if (Math.abs(this.bevelAngle - 90) > 0.1) {
                    ...
                    this.blade.rotation.x = bevelRadians * this.bevelDirection;  // Fixed: removed negative
                }
            }
        };
        ...
        this.setupViewCube(this.millEngine);
    ...
    setupManualCameraControls() {
        ...
        this.millCanvas.addEventListener('pointermove', (e) => {
            ...
            if (this.isDraggingBoard && this.currentBoard) {
                ...
                this.currentBoard.position.z += deltaX * 0.05;  // Fixed: positive for correct direction
                this.lastMouseX = e.clientX;
                return;
            }
    ...
    updateBladeTilt() {
        ...
        this.blade.rotation.x = tiltRadians * direction;  // Fixed: removed negative
        
        // Debug: Make blade more visible when beveled
        if (Math.abs(this.bevelAngle - 90) > 0.1) {
            console.log('Blade bevel applied:', this.bevelAngle, 'degrees, direction:', direction > 0 ? 'right' : 'left');
            console.log('Blade X rotation:', this.blade.rotation.x * 180 / Math.PI, 'degrees');
            // Make blade red when beveled for visibility
            if (!this.blade.material) {
                this.blade.material = new BABYLON.StandardMaterial('bladeMat', this.millScene);
            }
            this.blade.material.diffuseColor = new BABYLON.Color3(1, 0.2, 0.2);
            this.blade.material.alpha = 0.8;
        } else {
            if (this.blade.material) {
                this.blade.material.diffuseColor = new BABYLON.Color3(0.5, 0.5, 0.5);
                this.blade.material.alpha = 0.3;
            }
        }
    }
    ...
    executeCSGCut() {
        ...
        try {
            ...
            const bladeRadius = 254; // 20 inch blade (508mm diameter) for 500 inch length
            
            // Create a NEW blade mesh with bevel built into geometry
            ...
            // Bake BOTH rotations into geometry for CSG
            blade.bakeCurrentTransformIntoVertices();
            
            // Clear rotations since they're now baked into geometry
            blade.rotation.x = 0;
            blade.rotation.y = 0;
            
            console.log('Created CSG blade with both angles baked - Miter:', (this.bladeAngle * 180 / Math.PI).toFixed(1), 'Bevel:', this.bevelAngle);
            console.log('Created new CSG blade with bevel baked in:', this.bevelAngle, 'degrees');
            ...
            if (this.cutPieces && this.cutPieces.length > 0) {
                ...
                const sizeZ = maxZ - minZ;
                const maxSize = Math.max(sizeX, sizeZ);
                const paddedSize = maxSize * 1.2;  // 20% padding
    ...
    executeSimpleCut() {
        ...
        this.cutPieces.push(piece1);  // Single cut result
...
// Fix for bevel control degree labels - 90 at top, 0 on both sides
    ...
    */
    switchToBladeProfileView() {
        console.log("Switching to blade-eye view - rifle sight perspective");
        this.currentViewMode = "side";
        
        // Get current blade rotation (miter angle) and tilt (bevel)
        const bladeAngle = this.bladeAngle || 0;
        const bevelRadians = (90 - this.bevelAngle) * Math.PI / 180;
        const bevelDir = this.bevelDirection || 1;
        
        console.log("Blade miter:", bladeAngle * 180 / Math.PI, "degrees");
        console.log("Blade bevel:", this.bevelAngle, "degrees, direction:", bevelDir);
        
        // Calculate camera position at blade end, accounting for both miter and bevel
        const bladeLength = 260; // Slightly beyond blade end
        
        // Start with position for miter angle
        let cameraX = Math.cos(bladeAngle) * bladeLength;
        let cameraZ = Math.sin(bladeAngle) * bladeLength;
        let cameraY = 0;
        
        // Adjust Y position for bevel tilt
        if (Math.abs(this.bevelAngle - 90) > 0.1) {
            // Camera needs to tilt with blade bevel
            cameraY = Math.sin(bevelRadians) * bladeLength * 0.2; // Slight vertical offset for beveled view
        }
        
        // Calculate target position (opposite blade end)
        const targetX = -Math.cos(bladeAngle) * bladeLength;
        const targetZ = -Math.sin(bladeAngle) * bladeLength;
        let targetY = 0;
        
        // Adjust target for bevel
        if (Math.abs(this.bevelAngle - 90) > 0.1) {
            targetY = -Math.sin(bevelRadians) * bladeLength * 0.2;
        }
        
        // For ArcRotateCamera, calculate alpha, beta, radius from blade angle
        // Alpha is the rotation around Y (matches blade rotation)
        this.millCamera.alpha = bladeAngle - Math.PI/2;  // Perpendicular to blade
        console.log("Setting camera alpha to:", this.millCamera.alpha * 180 / Math.PI, "degrees");
        
        // Beta for looking horizontally (90 degrees from vertical)
        this.millCamera.beta = Math.PI/2;  // Horizontal view
        console.log("Setting camera beta to:", this.millCamera.beta * 180 / Math.PI, "degrees");
        
        // Adjust beta for bevel
        if (Math.abs(this.bevelAngle - 90) > 0.1) {
            this.millCamera.beta = Math.PI/2 - (bevelRadians * bevelDir * 0.3);
        }
        
        // Set radius (distance from target)
        this.millCamera.radius = 250;  // Distance to see full blade length
        console.log("Setting camera radius to:", this.millCamera.radius);
        
        // Force camera update
        this.millCamera.rebuildAnglesAndRadius();
        
        // Set target at blade center
        this.millCamera.target = new BABYLON.Vector3(0, 0, 0);
        
        // Switch to orthographic for precision
        this.millCamera.mode = BABYLON.Camera.ORTHOGRAPHIC_CAMERA;
        
        // Tighter view to focus on cutting area
        const viewSize = 30;
        const aspectRatio = this.millCanvas.width / this.millCanvas.height;
        this.millCamera.orthoLeft = -viewSize * aspectRatio;
        this.millCamera.orthoRight = viewSize * aspectRatio;
        this.millCamera.orthoTop = viewSize;
        this.millCamera.orthoBottom = -viewSize;
        
        // Lock camera in this position
    ...
    createAngleControl(id, label, minAngle, maxAngle, defaultAngle, onChange) {
        const container = document.createElement('div');
        container.style.cssText = 'display: flex; flex-direction: column; align-items: center; position: relative;';
        
        // Create gauge
        const gauge = document.createElement('div');
        gauge.id = id + '-gauge';
        gauge.style.cssText = 'width: 100px; height: 50px; position: relative; background: #222; border: 2px solid #555; border-radius: 50px 50px 0 0; overflow: hidden;';
        
        // Add gauge markings
        const markings = document.createElement('div');
        markings.style.cssText = 'position: absolute; width: 100%; height: 100%; pointer-events: none;';
        
        // Add major tick marks
        [-90, -45, 0, 45, 90].forEach(angle => {
            if (angle >= minAngle && angle <= maxAngle) {
                const tick = document.createElement('div');
                const rotation = angle + 90;  // Adjust for gauge orientation
                tick.style.cssText = 'position: absolute; width: 1px; height: 10px; background: #888; left: 50%; top: 0; transform-origin: center 50px; transform: translateX(-50%) rotate(' + rotation + 'deg);';
                markings.appendChild(tick);
            }
        });
        gauge.appendChild(markings);
        
        // Add needle
        const needle = document.createElement('div');
        needle.id = id + '-gauge-needle';
        needle.style.cssText = 'position: absolute; width: 2px; height: 45px; background: #ff9900; left: 50%; bottom: 0; transform-origin: center bottom; transform: translateX(-50%) rotate(' + defaultAngle + 'deg); transition: transform 0.3s;';
        gauge.appendChild(needle);
        
        // Add digital display
        const display = document.createElement('div');
        display.id = id + '-display';
        display.style.cssText = 'position: absolute; bottom: 5px; left: 50%; transform: translateX(-50%); color: #0ff; font-size: 14px; font-weight: bold;';
        display.textContent = defaultAngle + '°';
        gauge.appendChild(display);
        
        container.appendChild(gauge);
        
        // Add preset buttons
        const presets = document.createElement('div');
        presets.style.cssText = 'display: flex; gap: 5px; margin-top: 5px;';
        
        [-45, -22.5, 0, 22.5, 45].forEach(preset => {
            const btn = document.createElement('button');
            btn.style.cssText = 'padding: 2px 6px; background: #444; border: 1px solid #666; color: #ccc; font-size: 10px; cursor: pointer; border-radius: 3px;';
            btn.textContent = preset + '°';
            btn.onclick = () => {
                needle.style.transform = 'translateX(-50%) rotate(' + preset + 'deg)';
                display.textContent = preset + '°';
                onChange(preset);
            };
            presets.appendChild(btn);
        });
        ...
        const labelDiv = document.createElement('div');
        labelDiv.textContent = label;
        labelDiv.style.cssText = 'color: #ccc; font-size: 12px; margin-top: 5px;';
        container.appendChild(labelDiv);
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            const angle = Math.atan2(centerX - e.clientX, e.clientY - centerY) * 180 / Math.PI;
            const clampedAngle = Math.max(minAngle, Math.min(maxAngle, angle));
            needle.style.transform = 'translateX(-50%) rotate(' + clampedAngle + 'deg)';
            display.textContent = Math.round(clampedAngle) + '°';
            onChange(clampedAngle);
        });
