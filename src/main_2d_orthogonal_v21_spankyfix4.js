...
class CutListCAD {
    ...
    initWorkspace() {
        ...
        // SPANKY: Add ONE test block near origin for validation
        this.createTestBlock();
        
    }

    // SPANKY: Simple method to create ONE test block near origin for Master's validation
    createTestBlock() {
        
        // Create a simple box at position (2, 1, 0) - near origin but not overlapping
        const box = BABYLON.MeshBuilder.CreateBox("testBlock", {
            width: 1,
            height: 1,
            depth: 1
        }, this.scene);
        
        // Position it near origin
        box.position = new BABYLON.Vector3(2, 1, 0);
        
        // Give it a simple material so it's visible
        const material = new BABYLON.StandardMaterial("testBlockMaterial", this.scene);
        material.diffuseColor = new BABYLON.Color3(0.8, 0.3, 0.3); // Reddish color
    ...
    resizeCanvas() {
      ...
      switchMode(mode, options = {}) {
        ...
        if (this.camera) {
            ...
            if (mode === 'sketch') {
                // SPANKY: Force 2D orthogonal alignment and grid sync
                this.force2DOrthogonalAlignment();
                if (this.camera.force2DNoRotation) this.camera.force2DNoRotation();
            }
        }        if (mode === 'modeling') {
            ...
            if (this.grid) {
                // SPANKY FIX: FORCE orthogonal 2D camera alignment - NO rotation, grid-locked
                this.force2DOrthogonalAlignment();
                
                // Skip 3D camera sync - we want PURE orthogonal 2D regardless of previous 3D state
    ...
    startRenderLoop() {
      ...
      render2D() {
        ...
        // SPANKY: Always draw target sketch planes in 2D mode
        if (this.currentMode === 'sketch' && this.drawTargetSketchPlanes2D) {
            this.drawTargetSketchPlanes2D();
        }
        ...
        // SPANKY: Enforce 2D camera lock on every render
        if (this.camera && this.currentMode === 'sketch' && this.camera.force2DNoRotation) {
            this.camera.force2DNoRotation();
        }
    }
    
    // SPANKY: Draw the three default sketch planes as translucent panels or outlines in 2D
    drawTargetSketchPlanes2D() {
        if (!this.ctx || !this.camera) return;
        const planes = [
            { color: 'rgba(0,200,255,0.15)', border: 'rgba(0,200,255,0.7)', label: 'XY', x: 0, y: 0, w: 8, h: 8 },
            { color: 'rgba(255,200,0,0.10)', border: 'rgba(255,200,0,0.7)', label: 'YZ', x: -4, y: 0, w: 8, h: 8 },
            { color: 'rgba(0,255,100,0.10)', border: 'rgba(0,255,100,0.7)', label: 'XZ', x: 0, y: -4, w: 8, h: 8 }
        ];
        planes.forEach(plane => {
            // Convert world to screen
            const topLeft = this.camera.worldToScreen(plane.x, plane.y);
            const bottomRight = this.camera.worldToScreen(plane.x + plane.w, plane.y + plane.h);
            ...
            // Draw border
            this.ctx.strokeStyle = plane.border;
            this.ctx.lineWidth = 2;
            this.ctx.strokeRect(topLeft.x, topLeft.y, width, height);
            // Draw label
            this.ctx.font = 'bold 16px Arial';
            this.ctx.fillStyle = plane.border;
            this.ctx.fillText(plane.label, topLeft.x + 8, topLeft.y + 24);
            this.ctx.restore();
        });
    }
    ...
    handleExtrusionUpdate(eventData) {
        ...
        switch (this.extrudingObject.type) {
          ...
          if (newMesh && newTransformNode) {
            ...
            newMesh._isDirty = true;
            newMesh.markVerticesDataAsUpdatable(BABYLON.VertexBuffer.PositionKind, true);
            newMesh.markVerticesDataAsUpdatable(BABYLON.VertexBuffer.NormalKind, true);
            
            if (newMesh.geometry) {
                newMesh.geometry._boundingInfo = null;
    ...
    applySketchPlaneTransformation(transformNode, sketchPlane) {
        ...
        // Apply the correct orientation for each sketch plane
        // This ensures objects face the right direction and are EXACTLY positioned on the plane surface
        const gridType = sketchPlane.gridType;
        
        if (gridType === 'xy') {
            // XY plane (Front view) - default orientation (facing +Z)
            transformNode.rotation.x = 0;
            transformNode.rotation.y = 0;
            transformNode.rotation.z = 0;
            
        } else if (gridType === 'yz') {
            // YZ plane (Right view) - rotate to face +X direction
            transformNode.rotation.x = 0;
            transformNode.rotation.y = Math.PI / 2; // 90 degrees around Y
            transformNode.rotation.z = 0;
            
        } else if (gridType === 'xz') {
            // XZ plane (Top view) - rotate to face +Y direction  
            transformNode.rotation.x = -Math.PI / 2; // -90 degrees around X (face up)
            transformNode.rotation.y = 0;
            transformNode.rotation.z = 0;
        }
        
        
        // CRITICAL: Ensure transform node has no unintended offsets
        // The positioning will be handled separately by positionTransformNodeOnSketchPlane
        
        // IMPORTANT: Set the transform node's origin to be at the sketch plane center
        // This ensures objects rotate around the plane, not around world origin
        const planeOrigin = this.getSketchPlaneOrigin(sketchPlane);
    }
    ...
    // SPANKY V41: PRECISE coordinate mapping - Position transform node EXACTLY at specified 3D coordinates
    positionTransformNodeOnSketchPlane(transformNode, localPos, sketchPlane) {
        
        // CRITICAL FIX: PRECISE coordinate mapping from sketch plane 2D coordinates to exact 3D world coordinates
        // Each sketch plane defines a specific 2D coordinate system that maps to exact 3D positions
        
        if (sketchPlane.gridType === 'xy') {
            // XY plane: Local (u,v) maps directly to World (X,Y) with Z=0
            transformNode.position.x = localPos.x; // Local U → World X
            transformNode.position.y = localPos.y; // Local V → World Y  
            transformNode.position.z = 0.0; // Always on XY plane surface
            
        } else if (sketchPlane.gridType === 'yz') {
            // YZ plane: Local (u,v) maps to World (Y,Z) with X=0
            transformNode.position.x = 0.0; // Always on YZ plane surface
            transformNode.position.y = localPos.x; // Local U → World Y
            transformNode.position.z = localPos.y; // Local V → World Z
            
        } else if (sketchPlane.gridType === 'xz') {
            // XZ plane: Local (u,v) maps to World (X,Z) with Y=0
            transformNode.position.x = localPos.x; // Local U → World X
            transformNode.position.y = 0.0; // Always on XZ plane surface
            transformNode.position.z = localPos.y; // Local V → World Z
        }
    ...
    alignCameraToPlane(plane) {
        ...
        // Center camera on the plane's origin (or center)
        if (plane.origin && typeof plane.origin.x === 'number' && typeof plane.origin.y === 'number') {
            this.camera.x = plane.origin.x;
            this.camera.y = plane.origin.y;
        } else {
            // Default to world origin if no explicit origin
            this.camera.x = 0;
            this.camera.y = 0;
        }
        this.camera.rotation = 0;
        // Set zoom so the plane is clearly visible (fit 10 units vertically, for example)
        if (plane.height) {
            this.camera.zoom = this.camera.height / (plane.height * 1.2); // 20% margin
        } else {
            this.camera.zoom = 25; // fallback
        }
        if (this.camera.applyZoomConstraints) this.camera.applyZoomConstraints();
        if (this.camera.force2DNoRotation) this.camera.force2DNoRotation();
        // Keep target planes visible: do not hide or remove them here
        if (this.render2D) this.render2D();
    }    // SPANKY FIX: Force 2D camera to be perfectly orthogonal with consistent grid spacing
    force2DOrthogonalAlignment() {
        
        if (!this.camera) {
            return;
        }
        
        // CRITICAL: FORCE camera to orthogonal position - NO rotation allowed
        this.camera.rotation = 0; // ALWAYS straight up and down grids
        this.camera.x = 0; // Center on origin
        this.camera.y = 0; // Center on origin
        
        // SPANKY FIX: Calculate zoom to match 3D grid density
        // 3D grid uses gridScale=1.0 (1 world unit per grid line)
        // For 2D, we want similar visual density - around 20-30 pixels per inch for comfortable viewing
        const targetPixelsPerInch = 25; // Good density for 2D CAD work
        this.camera.zoom = targetPixelsPerInch;
        
        // IMPORTANT: Sync the 2D grid spacing to match the 3D grid 1:1
        if (this.grid) {
            // Force 2D grid to use 1-inch base spacing to match 3D gridScale=1.0
            this.grid.minorSpacing = 1.0; // 1 inch per minor grid line (matching 3D)
            this.grid.majorSpacing = 4.0; // 4 inches per major grid line (4x minor)
        }
        
        
        // Apply zoom constraints to ensure it's within valid range
        if (this.camera.applyZoomConstraints) {
            this.camera.applyZoomConstraints();
        }
    }
