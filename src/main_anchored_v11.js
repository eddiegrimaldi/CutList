...
// Cache-busting version: cache_bust_v11_ANCHORED_OBJECTS_FIX
...
class CutListCAD {
        ...
        try {
            ...
            switch (object.type) {
                case 'rectangle':
                    ...
                    // Create a simple plane mesh (no hardcoded rotations)
                    mesh = BABYLON.MeshBuilder.CreatePlane(`rect_${object.id}`, {
                        width: object.width,
                ...
                case 'circle':
                    ...
                    // Create a simple disc mesh (no hardcoded rotations)
                    mesh = BABYLON.MeshBuilder.CreateDisc(`circle_${object.id}`, {
    ...
    applySketchPlaneTransformation(transformNode, sketchPlane) {
        ...
        // This ensures objects face the right direction and are properly oriented
    ...
    // SPANKY V38: Position transform node properly on sketch plane surface
    positionTransformNodeOnSketchPlane(transformNode, localPos, sketchPlane) {
        
        // For proper CAD behavior, position objects directly on their sketch plane
        // Each plane has a specific positioning strategy to ensure objects are anchored
        
        if (sketchPlane.gridType === 'xy') {
            // XY plane (Front view) - objects at Z=0 (on the plane)
            transformNode.position.x = localPos.x;
            transformNode.position.y = localPos.y;
            transformNode.position.z = 0; // Anchored to XY plane at Z=0
            
        } else if (sketchPlane.gridType === 'yz') {
            // YZ plane (Right view) - objects at X=0 (on the plane)
            transformNode.position.x = 0; // Anchored to YZ plane at X=0
            transformNode.position.y = localPos.y;
            transformNode.position.z = -localPos.x; // Map local X to world -Z
            
        } else if (sketchPlane.gridType === 'xz') {
            // XZ plane (Top view) - objects at Y=0 (on the plane)
            transformNode.position.x = localPos.x;
            transformNode.position.y = 0; // Anchored to XZ plane at Y=0
