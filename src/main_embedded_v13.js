...
class CutListCAD {
            ...
            const transformNode = new BABYLON.TransformNode(`transform_${object.id}`, this.grid.scene);
              switch (object.type) {
                ...
                case 'circle':
                    ...
                    // SPANKY V40: Create ULTRA-THIN CYLINDER for precision anchoring - NOT a disc
              ...
              if (mesh) {
                ...
                // For ultra-thin shapes, offset by half the thickness in the extrusion direction
                mesh.position = BABYLON.Vector3.Zero();
                
                // Offset the mesh by half its thickness so bottom surface is exactly on plane
                if (object.type === 'rectangle') {
                    mesh.position.z = 0.005; // Half of 0.01 depth - bottom surface on plane
                } else if (object.type === 'circle') {
                    mesh.position.y = 0.005; // Half of 0.01 height - bottom surface on plane
                }
    ...
    handleExtrusionUpdate(eventData) {
        ...
        if (newMesh && newTransformNode) {
            ...
            // CRITICAL: Preserve rendering group to stay above grid
            newMesh.renderingGroupId = 1; // Objects render in foreground group
    ...
    getSketchPlaneOrigin(sketchPlane) {
      ...
      // SPANKY V39: FIXED anchoring - Position transform node EXACTLY on sketch plane surface
    positionTransformNodeOnSketchPlane(transformNode, localPos, sketchPlane) {
        
        // CRITICAL FIX: Objects must be positioned EXACTLY on their sketch plane surface
        // No floating or offsets - directly on the grid surface
        
        if (sketchPlane.gridType === 'xy') {
            // XY plane (Front view) - objects EXACTLY at Z=0 (on the plane surface)
            transformNode.position.x = localPos.x;
            transformNode.position.y = localPos.y;
            transformNode.position.z = 0.0; // EXACTLY on XY plane surface
            
        } else if (sketchPlane.gridType === 'yz') {
            // YZ plane (Right view) - objects EXACTLY at X=0 (on the plane surface)
            transformNode.position.x = 0.0; // EXACTLY on YZ plane surface
            transformNode.position.y = localPos.y;
            transformNode.position.z = -localPos.x; // Map local X to world -Z
            
        } else if (sketchPlane.gridType === 'xz') {
            // XZ plane (Top view) - objects EXACTLY at Y=0 (on the plane surface)
            transformNode.position.x = localPos.x;
            transformNode.position.y = 0.0; // EXACTLY on XZ plane surface
            transformNode.position.z = -localPos.y; // Map local Y to world -Z
        }
