...
// Cache-busting version: cache_bust_v10_FORCE_REFRESH_FIXED_ROTATION
...
class CutListCAD {
    ...
    startExtrusion(object) {
        ...
        try {
            ...
            if (mesh) {
                ...
                // Transform local position to world coordinates using Babylon.js methods
                const worldPos = this.transformPointToWorld(localPos, transform);
                
                // Apply world position to transform node
                transformNode.position = worldPos;
                
                // Apply rotation to transform node using proper Babylon.js matrix methods
    ...
    handleExtrusionUpdate(eventData) {
        ...
        switch (this.extrudingObject.type) {            case 'rectangle':
                ...
                // Recreate box with EXACT same parameter order as original flat mesh creation
                // Check if this is an XY plane object (which needs special handling)
                const isXYPlane = this.extrudingObject.sketchPlane && this.extrudingObject.sketchPlane.gridType === 'xy';                if (isXYPlane) {
                    // XY plane: Use CreateBox for extrusion (not Plane, since we need height)
                    newMesh = BABYLON.MeshBuilder.CreateBox(meshName, {
                        width: this.extrudingObject.width,
                        height: newHeight,
                        depth: this.extrudingObject.height,
                        updatable: true
                    }, this.grid.scene);
                } else {
                    // Other planes: Use original logic
                    newMesh = BABYLON.MeshBuilder.CreateBox(meshName, {
                        width: this.extrudingObject.width,
                        depth: this.extrudingObject.height,
                        height: newHeight,
