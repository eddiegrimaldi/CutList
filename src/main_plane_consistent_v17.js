...
class CutListCAD {
    ...
    applySketchPlaneTransformation(transformNode, sketchPlane) {
        ...
        // Use the plane's mathematical vectors for proper coordinate system mapping
        // This eliminates hardcoded rotations and ensures correct orientation for all planes
        const normal = sketchPlane.normal;
        const uAxis = sketchPlane.uAxis;  // Horizontal axis in plane (maps to mesh X)
        const vAxis = sketchPlane.vAxis;  // Vertical axis in plane (maps to mesh Y)
        
        
        // Create transformation matrix from plane coordinate system
        // The plane's uAxis becomes the mesh's local X axis
        // The plane's vAxis becomes the mesh's local Y axis  
        // The plane's normal becomes the mesh's local Z axis
        const transformMatrix = new BABYLON.Matrix();
        BABYLON.Matrix.FromValuesToRef(
            uAxis.x, uAxis.y, uAxis.z, 0,      // uAxis -> local X
            vAxis.x, vAxis.y, vAxis.z, 0,      // vAxis -> local Y
            normal.x, normal.y, normal.z, 0,   // normal -> local Z
            0, 0, 0, 1                         // Translation (none here)
        , transformMatrix);
        
        // Apply the transformation matrix to the transform node
        transformMatrix.decompose(null, transformNode.rotationQuaternion, null);
        
        // Ensure we're using quaternion rotation, not Euler angles        transformNode.rotation = BABYLON.Vector3.Zero();
