...
export class GridRenderer {
    ...
    setupScene() {
        ...
        // SPANKY ALERT TEST: Confirm this modified file is loading
        // alert('SPANKY ALERT: Modified grid.js is loading! If you see this alert, the cache bust worked!');

        // SPANKY DEBUG: Check if the Babylon canvas is actually visible
        if (this.babylonCanvas) {
        }
        
        // SPANKY TEST: Change background to bright color to verify scene is rendering
        this.scene.clearColor = new BABYLON.Color3(1, 0, 0); // Bright red background

        // SPANKY TEST: Create a MASSIVE test cube directly in the grid setup
        try {
            const gridTestCube = BABYLON.MeshBuilder.CreateBox("gridTestCube", {size: 50}, this.scene);
            gridTestCube.position = new BABYLON.Vector3(0, 0, 0); // Right at origin
            const gridTestMaterial = new BABYLON.StandardMaterial("gridTestMaterial", this.scene);
            gridTestMaterial.diffuseColor = new BABYLON.Color3(1, 1, 0); // Bright yellow
            gridTestMaterial.emissiveColor = new BABYLON.Color3(1, 1, 0); // Full yellow glow - should be impossible to miss
            gridTestCube.material = gridTestMaterial;
            
            // Make sure it's visible and not culled
            gridTestCube.isVisible = true;
            gridTestCube.setEnabled(true);
