    ...
    <script>
        function createBoard() {
            if (window.drawingWorld && window.drawingWorld.scene) {
                // Create a standard 2x4x96" board
                ...
                board.position.set(0, 2.54, 0); // Half height above grid
                ...
                // Create test router bit automatically
                const routerBitMesh = BABYLON.MeshBuilder.CreateBox("test_router_bit", {
                    width: 1.0,   // 10mm = 1.0 cm
                    height: 0.5,  // 5mm = 0.5 cm
                    depth: 1.0    // 10mm = 1.0 cm (cutting edge)
                }, window.drawingWorld.scene);
                
                routerBitMesh.position.set(10, 3, 0); // Position away from board
                
                // Router bit material
                const routerMaterial = new BABYLON.StandardMaterial("router_material", window.drawingWorld.scene);
                routerMaterial.diffuseColor = new BABYLON.Color3(0.7, 0.7, 0.8);
                routerMaterial.specularColor = new BABYLON.Color3(1, 1, 1);
                routerMaterial.specularPower = 100;
                routerBitMesh.material = routerMaterial;
                
                console.log("Test router bit created (10mm x 5mm x 10mm)");
            } else {
...
function showPreview(position, edge) {
    const scene = window.drawingWorld.scene;
    
    // Clear previous previews
    hidePreview();
    
    // Create red sphere for router bit position
    previewSphere = BABYLON.MeshBuilder.CreateSphere('previewSphere', {
        diameter: 2 // Visible size
    }, scene);
    previewSphere.position = position;
    const sphereMat = new BABYLON.StandardMaterial('sphereMat', scene);
    sphereMat.diffuseColor = new BABYLON.Color3(1, 0, 0);
    sphereMat.emissiveColor = new BABYLON.Color3(1, 0, 0);
    previewSphere.material = sphereMat;
    
    // Create orange tube along edge if edge data provided
    if (edge && edge.start && edge.end) {
        const path = [edge.start, edge.end];
        previewTube = BABYLON.MeshBuilder.CreateTube('previewTube', {
            path: path,
            radius: 0.5,
            tessellation: 8
        }, scene);
        const tubeMat = new BABYLON.StandardMaterial('tubeMat', scene);
        tubeMat.diffuseColor = new BABYLON.Color3(1, 0.5, 0);
        tubeMat.emissiveColor = new BABYLON.Color3(1, 0.5, 0);
        previewTube.material = tubeMat;
    }
}

function hidePreview() {
    if (previewSphere) {
        previewSphere.dispose();
        previewSphere = null;
    }
    if (previewTube) {
        previewTube.dispose();
...
// Simple face-based positioning
// Replace the selectEdgeForRouting function
// Visual preview helpers
...
// Simple face-based positioning
function handleFaceClick(pickInfo) {
    if (!pickInfo.hit || !pickInfo.pickedMesh || pickInfo.pickedMesh.name !== 'board') {
        return;
    }
    ...
    // Get world center and min/max
    const center = bounds.centerWorld;
    const min = bounds.minimumWorld;
    const max = bounds.maximumWorld;
    
    // Determine which face was clicked based on distance
    const faces = [
        { name: 'front', normal: new BABYLON.Vector3(0, 0, 1), center: new BABYLON.Vector3(center.x, center.y, max.z) },
        { name: 'back', normal: new BABYLON.Vector3(0, 0, -1), center: new BABYLON.Vector3(center.x, center.y, min.z) },
        { name: 'right', normal: new BABYLON.Vector3(1, 0, 0), center: new BABYLON.Vector3(max.x, center.y, center.z) },
        { name: 'left', normal: new BABYLON.Vector3(-1, 0, 0), center: new BABYLON.Vector3(min.x, center.y, center.z) },
        { name: 'top', normal: new BABYLON.Vector3(0, 1, 0), center: new BABYLON.Vector3(center.x, max.y, center.z) },
        { name: 'bottom', normal: new BABYLON.Vector3(0, -1, 0), center: new BABYLON.Vector3(center.x, min.y, center.z) }
    ];
    ...
    faces.forEach(face => {
        const distance = BABYLON.Vector3.Distance(clickPoint, face.center);
    ...
    console.log('Clicked face:', closestFace.name);
    
    // Calculate upper-left corner of the face
    let cornerPosition;
    switch(closestFace.name) {
        case 'front':
            cornerPosition = new BABYLON.Vector3(min.x, max.y, max.z);
            break;
        case 'back':
            cornerPosition = new BABYLON.Vector3(max.x, max.y, min.z);
            break;
        case 'right':
            cornerPosition = new BABYLON.Vector3(max.x, max.y, min.z);
            break;
        case 'left':
            ...
            break;
        case 'top':
            cornerPosition = new BABYLON.Vector3(min.x, max.y, min.z);
            break;
        case 'bottom':
            cornerPosition = new BABYLON.Vector3(min.x, min.y, max.z);
            break;
    }
    
    // Position router bit
    const routerBit = window.drawingWorld.scene.getMeshByName('test_router_bit');
    if (routerBit) {
        // Offset position slightly away from face
        const offset = closestFace.normal.scale(0.5);
        routerBit.position = cornerPosition.add(offset);
        
        // Rotate to face the correct direction
        routerBit.lookAt(routerBit.position.add(closestFace.normal));
        
        console.log(`Router bit positioned at ${closestFace.name} face upper-left corner`);
    }
}
...
function showEdgePreview(edge) {
    hideEdgePreview();
    
    const scene = window.drawingWorld.scene;
    const path = [edge.start, edge.end];
    
    edgePreview = BABYLON.MeshBuilder.CreateTube('edgePreview', {
        path: path,
        radius: 0.2,
        tessellation: 16
    }, scene);
    
    const mat = new BABYLON.StandardMaterial('edgeMat', scene);
    mat.diffuseColor = new BABYLON.Color3(1, 0.5, 0);
    mat.emissiveColor = new BABYLON.Color3(1, 0.5, 0);
    edgePreview.material = mat;
}
...
function getClosestEdge(point, mesh) {
    const bounds = mesh.getBoundingInfo().boundingBox;
    const min = bounds.minimumWorld;
    const max = bounds.maximumWorld;
    ...
    const edges = [
        // Top edges
        {start: new BABYLON.Vector3(min.x, max.y, min.z), end: new BABYLON.Vector3(max.x, max.y, min.z)},
        {start: new BABYLON.Vector3(max.x, max.y, min.z), end: new BABYLON.Vector3(max.x, max.y, max.z)},
        {start: new BABYLON.Vector3(max.x, max.y, max.z), end: new BABYLON.Vector3(min.x, max.y, max.z)},
        {start: new BABYLON.Vector3(min.x, max.y, max.z), end: new BABYLON.Vector3(min.x, max.y, min.z)},
        // Bottom edges
        {start: new BABYLON.Vector3(min.x, min.y, min.z), end: new BABYLON.Vector3(max.x, min.y, min.z)},
        {start: new BABYLON.Vector3(max.x, min.y, min.z), end: new BABYLON.Vector3(max.x, min.y, max.z)},
        {start: new BABYLON.Vector3(max.x, min.y, max.z), end: new BABYLON.Vector3(min.x, min.y, max.z)},
        {start: new BABYLON.Vector3(min.x, min.y, max.z), end: new BABYLON.Vector3(min.x, min.y, min.z)},
        // Vertical edges
        {start: new BABYLON.Vector3(min.x, min.y, min.z), end: new BABYLON.Vector3(min.x, max.y, min.z)},
        {start: new BABYLON.Vector3(max.x, min.y, min.z), end: new BABYLON.Vector3(max.x, max.y, min.z)},
        {start: new BABYLON.Vector3(max.x, min.y, max.z), end: new BABYLON.Vector3(max.x, max.y, max.z)},
        {start: new BABYLON.Vector3(min.x, min.y, max.z), end: new BABYLON.Vector3(min.x, max.y, max.z)}
    ];
    ...
    edges.forEach(edge => {
        const dist = distanceToLineSegment(point, edge.start, edge.end);
        if (dist < minDist) {
            minDist = dist;
            closestEdge = edge;
        }
    ...
    return minDist < 2 ? closestEdge : null;
}
...
function distanceToLineSegment(point, start, end) {
    ...
    const closest = start.add(line.scale(t));
    return BABYLON.Vector3.Distance(point, closest);
}    if (window.drawingWorld && window.drawingWorld.scene) {
        console.log('Face selection mode enabled - click on board face');
        
        // Remove any existing observers
        window.drawingWorld.scene.onPointerObservable.clear();
        
        // Add mouse move observer for preview
        window.drawingWorld.scene.onPointerObservable.add((pointerInfo) => {
            if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERMOVE) {                const pickInfo = window.drawingWorld.scene.pick(                    window.drawingWorld.scene.pointerX,                    window.drawingWorld.scene.pointerY                );                                if (pickInfo.hit && pickInfo.pickedMesh && pickInfo.pickedMesh.name === "board") {                    const edge = getClosestEdge(pickInfo.pickedPoint, pickInfo.pickedMesh);                    if (edge) {                        showEdgePreview(edge);                    } else {                        hideEdgePreview();                    }                } else {                    hideEdgePreview();
                }
            } else if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERUP) {
                const pickInfo = pointerInfo.pickInfo;
                handleFaceClick(pickInfo);
            }
        });
    } else {
        alert('Scene not ready');
...
window.createBoard = function() {
    ...
    // Remove existing
    const existing = scene.getMeshByName('board');
    if (existing) existing.dispose();
    const existingBit = scene.getMeshByName('imported_router_bit');
    if (existingBit) existingBit.dispose();
    
    // Create 2x2x30 board
    const board = BABYLON.MeshBuilder.CreateBox('board', {
        width: 5.08, height: 5.08, depth: 76.2
    }, scene);
    board.position.set(0, 2.54, 0);
    ...
    // Create router bit
    const routerBit = BABYLON.MeshBuilder.CreateBox('imported_router_bit', {
        width: 1.0, height: 0.2, depth: 1.0
    }, scene);
    routerBit.position.set(0, 8, 0);
    
    const bitMaterial = new BABYLON.StandardMaterial('bitMaterial', scene);
    bitMaterial.diffuseColor = new BABYLON.Color3(1, 0, 0);
    routerBit.material = bitMaterial;
    
    console.log('2x2x30 board and router bit created');
};
