...
<body>
    ...
    <script>
        ...
        function selectEdgeForRouting() {
            if (window.drawingWorld && window.drawingWorld.scene) {
                console.log('Edge selection mode enabled - click on board edge');
                    ...
                    if (pointerInfo.type === BABYLON.PointerEventTypes.POINTERUP) {
                        ...
                        if (pickInfo.hit && pickInfo.pickedMesh) {
                            ...
                            if (mesh.name === 'board') {
                                ...
                                // Find the closest edge to the clicked point
                                const clickPoint = pickInfo.pickedPoint;
                                const edge = findClosestEdge(mesh, clickPoint);
                                
                                if (edge && window.drawingWorld.routerBitSystem) {
                                    // Add the missing method directly
                                    if (!window.drawingWorld.routerBitSystem.positionRouterBitOnEdge) {
                                        window.drawingWorld.routerBitSystem.positionRouterBitOnEdge = function(edge, mesh, clickPos) {
                                            console.log('🎯 Camera-facing edge detection - proper implementation');
                                            ...
                                            // Get mesh bounds for face calculations
                                            const bounds = mesh.getBoundingInfo();
                                            const center = bounds.centerWorld;
                                            
                                            // Define all 6 faces with their normals and positions
                                            const faces = [
                                                { name: 'front', normal: new BABYLON.Vector3(0, 0, 1), center: new BABYLON.Vector3(center.x, center.y, bounds.maximum.z) },
                                                { name: 'back', normal: new BABYLON.Vector3(0, 0, -1), center: new BABYLON.Vector3(center.x, center.y, bounds.minimum.z) },
                                                { name: 'right', normal: new BABYLON.Vector3(1, 0, 0), center: new BABYLON.Vector3(bounds.maximum.x, center.y, center.z) },
                                                { name: 'left', normal: new BABYLON.Vector3(-1, 0, 0), center: new BABYLON.Vector3(bounds.minimum.x, center.y, center.z) },
                                                { name: 'top', normal: new BABYLON.Vector3(0, 1, 0), center: new BABYLON.Vector3(center.x, bounds.maximum.y, center.z) },
                                                { name: 'bottom', normal: new BABYLON.Vector3(0, -1, 0), center: new BABYLON.Vector3(center.x, bounds.minimum.y, center.z) }
                                            ];
                                            
                                            // Find edge's 2 adjacent faces by checking which faces the clicked point is closest to
                                            let adjacentFaces = [];
                                            const clickTolerance = 0.5; // Tolerance for edge detection
                                            
                                            faces.forEach(face => {
                                                const distanceToFace = Math.abs(BABYLON.Vector3.Dot(
                                                    clickPos.subtract(face.center), 
                                                    face.normal
                                                ));
                                                
                                                if (distanceToFace < clickTolerance) {
                                                    adjacentFaces.push(face);
                                                }
                                            });
                                            
                                            // If we found edge's adjacent faces, pick the most camera-facing one
                                            let selectedFace = null;
                                            let maxAlignment = -2; // Start with very negative value
                                            
                                            if (adjacentFaces.length >= 2) {
                                                console.log('Edge detected between faces:', adjacentFaces.map(f => f.name));
                                                
                                                adjacentFaces.forEach(face => {
                                                    // Calculate alignment: face normal dot camera direction
                                                    // Negative dot product means face is pointing toward camera
                                                    const alignment = -BABYLON.Vector3.Dot(face.normal, cameraDirection);
                                                    console.log(`Face ${face.name} alignment:`, alignment);
                                                ...
                                                console.log('Edge detection failed, using closest face to click');
                                                // Fallback: find closest face to click point
                                                let minDistance = Infinity;
                                                faces.forEach(face => {
                                                    const distance = BABYLON.Vector3.Distance(clickPos, face.center);
                                            ...
                                            console.log('Selected camera-facing face:', selectedFace.name);
                                            
                                            // Calculate top-left corner of selected face
                                            let topLeftCorner;
                                            
                                            switch (selectedFace.name) {
                                                case 'front':
                                                    topLeftCorner = new BABYLON.Vector3(bounds.minimum.x, bounds.maximum.y, bounds.maximum.z);
                                                    break;
                                                case 'back':
                                                    topLeftCorner = new BABYLON.Vector3(bounds.maximum.x, bounds.maximum.y, bounds.minimum.z);
                                                    break;
                                                case 'right':
                                                    topLeftCorner = new BABYLON.Vector3(bounds.maximum.x, bounds.maximum.y, bounds.maximum.z);
                                                    break;
                                                case 'left':
                                                    topLeftCorner = new BABYLON.Vector3(bounds.minimum.x, bounds.maximum.y, bounds.minimum.z);
                                                    break;
                                                case 'top':
                                                    topLeftCorner = new BABYLON.Vector3(bounds.minimum.x, bounds.maximum.y, bounds.maximum.z);
                                                    break;
                                                case 'bottom':
                                                    topLeftCorner = new BABYLON.Vector3(bounds.minimum.x, bounds.minimum.y, bounds.maximum.z);
                                                    break;
                                            }
                                            
                                            console.log('Router bit positioned at top-left corner of', selectedFace.name, 'face:', topLeftCorner);
                                            
                                            return {
                                                position: topLeftCorner,
                                                faceNormal: selectedFace.normal,
                                                selectedFace: selectedFace.name,
                                                cameraAlignment: maxAlignment
                                            };
                                        };
                                    }
                                    
                                    // Position router bit using camera-facing logic
                                    const routerBitData = window.drawingWorld.routerBitSystem.positionRouterBitOnEdge(edge, mesh, clickPoint);
                                    
                                    if (routerBitData) {
                                        // Apply position to actual router bit mesh
                                        const routerBit = window.drawingWorld.scene.getMeshByName('imported_router_bit');
                                        if (routerBit) {
                                            routerBit.position = routerBitData.position;
                                            
                                            // Move camera to look at the positioned router bit
                                            const camera = window.drawingWorld.scene.activeCamera;
                                            if (camera) {
                                                camera.setTarget(routerBit.position);
                                            }
                                            
                                            console.log('Router bit positioned and camera focused on:', routerBit.position);
                                        } else {
                                            alert('No router bit found - upload one first');
                                        }
                                    }
